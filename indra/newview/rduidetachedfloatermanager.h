/**
 * @file rduidetachedfloatermanager.h
 * @brief
 *
...
public:
    ...
    virtual bool open(const NativeRect& rect, float scale_multiplier, const std::optional<Vec2>& drag_offset,
                      const std::optional<Vec2>& logical_size = std::nullopt, const std::optional<NativePoint>& drag_cursor = std::nullopt) = 0;
    ...
    virtual Floater& replaceFloater(std::unique_ptr<Floater> replacement, const std::optional<Vec2>& logical_size) = 0;
...
public:
    ...
    public:
        ...
        virtual std::optional<NativePoint> releasePointerForDetach(const Vec2& main_position) = 0;
    };

    using PresentationFactory = std::function<std::unique_ptr<DetachedFloaterPresentation>(std::unique_ptr<Floater>&)>;

    DetachedFloaterManager(Surface& attached_surface, FloaterPlacementStore& placement_store, PresentationFactory presentation_factory,
    ...
    void requestDetach(const FloaterInstanceId& identity, Floater& floater, const Vec2& desired, const Vec2& drag_offset);
    ...
    Floater* replace(Floater& current, std::unique_ptr<Floater> replacement, const std::optional<Vec2>& logical_size);
    bool restore(const FloaterInstanceId& identity, Floater& floater, const DetachedFloaterPlacement& placement);
