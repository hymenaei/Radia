...
namespace rdui::viewer
{
    struct DetachedFloaterManager::Impl
    {
        ...
        Impl(Surface& attached_surface,
             FloaterPlacementStore& placement_store,
             PresentationFactory presentation_factory,
             Environment& environment)
            : surface(attached_surface), placements(placement_store),
              makePresentation(std::move(presentation_factory)), environment(environment)
        {
        }

        DetachedEntry* owner(const Floater& floater)
        {
            const auto found = std::find_if(detached.begin(), detached.end(), [&floater](const DetachedEntry& entry)
            {
                return entry.presentation->floater() == &floater;
            });
        ...
        void saveDetached(const DetachedEntry& entry)
        {
            ...
            placements.save(entry.identity, DetachedFloaterPlacement{
                rect.x, rect.y, rect.width, rect.height, presentation.monitorId(), logical_size});
        }
        ...
        void detachPendingFloater()
        {
            ...
            else if (!presentation->open(environment.mainRectToNative(desired),
                                         environment.nativeScaleMultiplier(),
                                         pending->dragOffset, std::nullopt, drag_cursor))
            {
        ...
        void reattach(std::size_t index, bool preserve_minimized)
        {
            ...
            const Vec2 size = floater->canResize()
                ? Vec2{floater->rect().w, floater->rect().h}
                : surface.preferredFloaterSize(*floater);
    ...
    DetachedFloaterManager::DetachedFloaterManager(Surface& attached_surface,
                                                   FloaterPlacementStore& placement_store,
                                                   PresentationFactory presentation_factory,
                                                   Environment& environment)
        : mImpl(std::make_unique<Impl>(attached_surface, placement_store,
                                      std::move(presentation_factory), environment))
    {
    }

    DetachedFloaterManager::~DetachedFloaterManager() = default;

    void DetachedFloaterManager::requestDetach(const FloaterInstanceId& identity, Floater& floater,
    ...
    {
        if (Impl::DetachedEntry* entry = mImpl->owner(floater))
            entry->presentation->applyResize(logical_rect);
    }
    ...
    Floater* DetachedFloaterManager::replace(Floater& current, std::unique_ptr<Floater> replacement,
                                         ...
                                         const DetachedFloaterPlacement& placement)
    {
        ...
        if (detached_floater->canResize() && placement.logicalSize)
            logical_size = {placement.logicalSize->width, placement.logicalSize->height};
        if (presentation->open(native, mImpl->environment.nativeScaleMultiplier(),
                               std::nullopt, logical_size))
        {
    ...
    {
        if (const Impl::DetachedEntry* entry = mImpl->owner(floater))
            mImpl->saveDetached(*entry);
    }
}
