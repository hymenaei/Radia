        ...
        return defineWidget<Fieldset>(Fieldset::ELEMENT)
                            ...
                            ViewBuildResult& result, const std::string& source)
            {
                ...
                for (const auto& child : fieldset.children())
                {
                    ...
                    else result.error("view.fieldset.child_unsupported",
                                      "Fieldset accepts only one Legend and direct Fields.",
                                      source, element.source().begin.line, element.source().begin.column);
                }
                if (!fieldset.legend())
                    result.error("view.fieldset.legend_required",
                                 "Fieldset requires exactly one direct Legend.",
                                 source, element.source().begin.line, element.source().begin.column);
                if (field_count == 0)
                    result.error("view.fieldset.field_required",
                                 "Fieldset requires one or more direct Fields.",
                                 source, element.source().begin.line, element.source().begin.column);
                if (has_flow_break)
                    result.error("view.fieldset.flow_break_unsupported",
            ...
            .scopedInlineContent("legend", {InlineContentKind::B, InlineContentKind::I, InlineContentKind::S,
                   ...
                   const std::string& source, std::size_t line, std::size_t column) -> Widget*
                {
                    if (fieldset.legend())
                    {
                        result.error("view.fieldset.legend_duplicate",
                                     "Fieldset accepts only one Legend.",
                                     source, line, column);
