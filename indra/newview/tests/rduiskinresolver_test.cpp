...
namespace tut
{
    namespace
    {
        ...
        std::string manifest(const std::string& id, const std::string& base,
                             const std::string& radia)
        {
            return "{\n"
                   "  \"id\": \"" + id + "\",\n"
                   "  \"name\": \"Test\",\n"
...
locales:
  en:
    ...
    struct skin_resolver
    {
        skin_resolver()
        {
            root = std::filesystem::temp_directory_path()
                / ("rdui-skin-resolver-" + std::to_string(
    ...
    template<> template<>
    void skin_resolver_object::test<1>()
    {
...
locales:
  en:
    ...
    template<> template<>
    void skin_resolver_object::test<2>()
    {
    ...
    template<> template<>
    void skin_resolver_object::test<3>()
    {
    ...
    template<> template<>
    void skin_resolver_object::test<4>()
    {
    ...
    template<> template<>
    void skin_resolver_object::test<5>()
    {
    ...
    template<> template<>
    void skin_resolver_object::test<6>()
    {
        ...
        write(selected / "manifest.json", manifest(
            "test.selected", "\"test.base\"", "\"layouts\": \"../outside\""));

        const auto result = resolver.resolve(selected, {root});

        ensure("traversal rejected", !result.ok());
        ensure("stable traversal diagnostic", std::any_of(
            result.errors.begin(), result.errors.end(), [](const rdui::Diagnostic& diagnostic)
            {
    ...
    void skin_resolver_object::test<7>()
    {
        ...
        ensure_equals("entrypoint identity is Skin-relative", layers.front().entrypoint,
                      std::string("rdui/skin.radia"));
        ensure("imported module captured",
               layers.front().modules.contains("rdui/styles/panel.radia"));
        ensure("complete generation with import compiles",
               rdui::SkinCompiler().prepare(resolved.snapshot).ok());
    }

    template<> template<>
    void skin_resolver_object::test<8>()
    {
        ...
        ensure_equals("no Base module fallback diagnostic", compiled.errors.front().code,
                      std::string("stylesheet.import.missing"));
        ensure_equals("diagnostic identifies derived entrypoint", compiled.errors.front().source,
                      std::string("test.derived/rdui/skin.radia"));
    }

    template<> template<>
    void skin_resolver_object::test<9>()
    {
        ...
        ensure("variant module is captured",
               layers.front().modules.contains("rdui/variants.radia"));
        const auto compiled = rdui::SkinCompiler().prepare(resolved.snapshot);
        const std::string failure = compiled.errors.empty() ? std::string("no diagnostic")
                                                            : compiled.errors.front().formatted();
