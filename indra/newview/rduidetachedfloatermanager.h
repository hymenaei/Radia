...
namespace rdui::viewer
{
    class DetachedFloaterPresentation
    {
        public:
            ...
            virtual bool open(const RduiNativeRect& rect, float scale_multiplier,
                              const std::optional<Vec2>& drag_offset,
                              const std::optional<Vec2>& logical_size = std::nullopt,
                              const std::optional<RduiNativePoint>& drag_cursor = std::nullopt) = 0;
            ...
            virtual RduiNativeRect nativeRect() const = 0;
        ...
        public:
            class Environment
            {
                public:
                    ...
                    virtual RduiNativeRect mainRectToNative(const Rect& rect) const = 0;
                    virtual float nativeScaleMultiplier() const = 0;
                    virtual Vec2 nativeBottomLeftInMain(const RduiNativeRect& rect) const = 0;
                    virtual bool nativePointInsideMain(const Vec2& point) const = 0;
                    virtual bool placementVisible(const RduiNativeRect& rect,
                                                  const std::string& monitor_id) const = 0;
                    virtual std::optional<RduiNativePoint> releasePointerForDetach(
