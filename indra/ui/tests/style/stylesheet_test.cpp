...
template<> template<> void stylesheetObject::test<1>() {
    ...
    const radia::ui::Style button = stylesheet.resolve("button", "", {}, 0);
    ensure_approximately_equals("hsl color token", button.backgroundColor.g, 1.f, 6);
    radia::ui::Button control;
    radia::ui::Label& label = control.setLabel("Inherited");
    ensure_approximately_equals("rgb color token with alpha inherits into button label", radia::ui::resolveWidgetStyle(stylesheet, label).textColor.a, .5f,
                                6);
    ensure_equals("button caption inherits button font size", radia::ui::resolveWidgetStyle(stylesheet, label).fontSize, 17.f);
    radia::ui::Label standalone("Standalone");
    ensure_equals("standalone Label keeps its own font size", radia::ui::resolveWidgetStyle(stylesheet, standalone).fontSize, 29.f);
    const radia::ui::StyleSheetLoadResult invalid = stylesheet.loadRadia("switch { background-color: ##invalid; }", "invalid.radia");
...
template<> template<> void stylesheetObject::test<2>() {
    ...
    const radia::ui::Style style = stylesheet.resolve("panel", "", {}, 0);
...
template<> template<> void stylesheetObject::test<3>() {
    ...
    const uint8_t focused = static_cast<uint8_t>(radia::ui::WidgetState::Focused);
    const uint8_t focusVisible = radia::ui::WidgetState::Focused | radia::ui::WidgetState::FocusVisible;
    ...
    ensure_equals("orphan focus-visible state does not match",
                  stylesheet.resolve("button", "", {}, static_cast<uint8_t>(radia::ui::WidgetState::FocusVisible)).borderWidth.top, 1.f);
}

template<> template<> void stylesheetObject::test<4>() {
    radia::ui::StyleSheet stylesheet;
    ensure("initial stylesheet loads", stylesheet.loadRadia("button { width: 12px; }").ok());
    const radia::ui::StyleSheetLoadResult failed = stylesheet.loadRadia("button { width: 99px; unknown-property: 1; }", "candidate.radia");
...
template<> template<> void stylesheetObject::test<5>() {
    radia::ui::StyleSheet stylesheet;
    const radia::ui::StyleSheetLoadResult malformed = stylesheet.loadRadia(":root { --bad: nonsense; }", "tokens.radia");
    ...
    const radia::ui::StyleSheetLoadResult missing = stylesheet.loadRadia("button { width: var(--missing); }", "missing-token.radia");
...
template<> template<> void stylesheetObject::test<6>() {
    ...
    ensure_equals("child uses owner class", radia::ui::resolveWidgetStyle(stylesheet, icon).width.pixels(), 10.f);
    radia::ui::detail::WidgetCompilerAccess::setState(button, radia::ui::WidgetState::Hovered, true);
    ensure_equals("child uses owner state", radia::ui::resolveWidgetStyle(stylesheet, icon).width.pixels(), 18.f);
}
...
template<> template<> void stylesheetObject::test<7>() {
    ...
    radia::ui::detail::WidgetCompilerAccess::setState(floater, radia::ui::WidgetState::Hovered, true);
    ensure_equals("owner hover does not hover interactive part", radia::ui::resolveWidgetStyle(stylesheet, *floater.closeButton()).width.pixels(), 10.f);
    radia::ui::detail::WidgetCompilerAccess::setState(*floater.closeButton(), radia::ui::WidgetState::Hovered, true);
    ensure_equals("interactive part uses its own hover state", radia::ui::resolveWidgetStyle(stylesheet, *floater.closeButton()).width.pixels(), 18.f);
}
...
template<> template<> void stylesheetObject::test<8>() {
    ...
    ensure_equals("pointer cursor is typed", static_cast<int>(stylesheet.resolve("button", "", {}, 0).cursor),
                  static_cast<int>(radia::ui::CursorStyle::Pointer));
    ...
    const radia::ui::StyleSheetLoadResult invalid = stylesheet.loadRadia("button { cursor: teleport; }", "cursor.radia");
...
template<> template<> void stylesheetObject::test<9>() {
    ...
    const radia::ui::StyleSheetLoadResult valid = stylesheet.loadRadia(kValidStyles);
    ...
    auto rejects = [&](const std::string& radia, const std::string& code) {
        const radia::ui::StyleSheetLoadResult result = stylesheet.loadRadia(radia, "contract.radia");
    ...
    const radia::ui::StyleSheetLoadResult deadStates =
...
template<> template<> void stylesheetObject::test<10>() {
    ...
    auto inherited = std::make_unique<radia::ui::Label>("Inherited");
    radia::ui::Label* inheritedLabel = inherited.get();
    parent->addChild(std::move(inherited));
    auto overridden = std::make_unique<radia::ui::Label>("Overridden");
    radia::ui::Label* overriddenLabel = overridden.get();
    overridden->setId("override");
    parent->addChild(std::move(overridden));

    const radia::ui::Style inheritedStyle = radia::ui::resolveWidgetStyle(stylesheet, *inheritedLabel);
    ensure_equals("font family inherits", static_cast<int>(inheritedStyle.fontFamily), static_cast<int>(radia::ui::FontFamily::Sans));
    ...
    ensure_equals("text alignment inherits", static_cast<int>(inheritedStyle.textAlign), static_cast<int>(radia::ui::TextAlign::Center));
    ...
    ensure_equals("cursor inherits", static_cast<int>(inheritedStyle.cursor), static_cast<int>(radia::ui::CursorStyle::Grab));
    ...
    ensure_equals("pointer event policy does not inherit", static_cast<int>(inheritedStyle.pointerEvents),
                  static_cast<int>(radia::ui::PointerEvents::Default));
    ensure_equals("background does not inherit", inheritedStyle.backgroundColor.a, 0.f);

    const radia::ui::Style overriddenStyle = radia::ui::resolveWidgetStyle(stylesheet, *overriddenLabel);
    ...
    ensure_equals("explicit child cursor overrides inheritance", static_cast<int>(overriddenStyle.cursor),
                  static_cast<int>(radia::ui::CursorStyle::Default));
}
...
template<> template<> void stylesheetObject::test<11>() {
    ...
    const radia::ui::Style split = stylesheet.resolve("panel", "", {}, 0);
    ensure_equals("overflow shorthand sets horizontal axis", static_cast<int>(split.overflowX), static_cast<int>(radia::ui::Overflow::Hidden));
    ensure_equals("overflow shorthand accepts a distinct vertical axis", static_cast<int>(split.overflowY),
                  static_cast<int>(radia::ui::Overflow::Visible));
    const radia::ui::Style overridden = stylesheet.resolve("panel", "visible", {}, 0);
    ...
    ensure_equals("overflow longhand overrides shorthand vertical value", static_cast<int>(overridden.overflowY),
                  static_cast<int>(radia::ui::Overflow::Hidden));

    const radia::ui::StyleSheetLoadResult invalid = stylesheet.loadRadia("panel { overflow: scroll; }", "overflow.radia");
...
template<> template<> void stylesheetObject::test<12>() {
    ...
    ensure_equals("expanded header retains content separator", radia::ui::resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 1.f);
    floater.setMinimized(true);
    ensure("minimize projects widget state", floater.hasState(radia::ui::WidgetState::Minimized));
    ensure_equals("minimized owner state removes header separator", radia::ui::resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 0.f);
    floater.setMinimized(false);
    ensure("restore clears widget state", !floater.hasState(radia::ui::WidgetState::Minimized));
    ensure_equals("restored header regains content separator", radia::ui::resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 1.f);
}

template<> template<> void stylesheetObject::test<13>() {
    const radia::ui::Style initial;
    ...
    const radia::ui::Style automatic = stylesheet.resolve("panel", "", {}, 0);
    ensure("width can be reset to auto", automatic.width.isAuto());
    ensure("height can be reset to auto", automatic.height.isAuto());
    const radia::ui::Style automaticSize = stylesheet.resolve("button", "", {}, 0);
    ...
    const radia::ui::Style mixedSize = stylesheet.resolve("icon", "", {}, 0);
    ...
    const radia::ui::Style automaticGap = stylesheet.resolve("panel", "", {}, 0);
...
template<> template<> void stylesheetObject::test<14>() {
    ...
    radia::ui::Panel root;
    root.addClass("root");
    auto direct = std::make_unique<radia::ui::Label>("direct");
    direct->addClass("direct");
    radia::ui::Label* directLabel = direct.get();
    root.addChild(std::move(direct));

    auto container = std::make_unique<radia::ui::Panel>();
    auto nested = std::make_unique<radia::ui::Label>("nested");
    nested->addClass("nested");
    radia::ui::Label* nestedLabel = nested.get();
    container->addChild(std::move(nested));
    root.addChild(std::move(container));

    const radia::ui::Style directStyle = radia::ui::resolveWidgetStyle(stylesheet, *directLabel);
    ...
    const radia::ui::Style nestedStyle = radia::ui::resolveWidgetStyle(stylesheet, *nestedLabel);
...
template<> template<> void stylesheetObject::test<15>() {
    ...
    ensure("linear gradient kind is typed", style.backgroundGradient->kind == radia::ui::GradientKind::Linear);
    ...
    ensure("dashed outline style is typed", stylesheet.resolve("label", "", {}, 0).outline.style == radia::ui::OutlineStyle::Dashed);
    ...
    const radia::ui::Style effects = stylesheet.resolve("panel", "", {}, 0);
    ensure_equals("effect list preserves both functions", effects.effects.size(), 2U);
    ensure("first effect targets background", effects.effects[0].kind == radia::ui::EffectKind::BackgroundBlur);
    ...
    ensure("second effect targets layer", effects.effects[1].kind == radia::ui::EffectKind::LayerBlur);
    ...
    const radia::ui::Style radial = stylesheet.resolve("panel", "", {}, 0);
    ensure("radial background is typed", radial.backgroundGradient && radial.backgroundGradient->kind == radia::ui::GradientKind::Radial);
    ensure("radial shape is typed", radial.backgroundGradient->radialShape == radia::ui::RadialGradientShape::Circle);
    ...
    ensure("conic border-color is typed", radial.borderGradient && radial.borderGradient->kind == radia::ui::GradientKind::Conic);
    ...
    const radia::ui::Style repeatingBorder = stylesheet.resolve("button", "", {}, 0);
    ...
    const radia::ui::Style repeatingRadial = stylesheet.resolve("switch", "", {}, 0);
    ensure("repeating radial gradient is typed",
           repeatingRadial.backgroundGradient
               && repeatingRadial.backgroundGradient->kind == radia::ui::GradientKind::Radial
               && repeatingRadial.backgroundGradient->repeating);
    const radia::ui::Style repeatingConic = stylesheet.resolve("floater", "", {}, 0);
    ensure("repeating conic gradient is typed",
           repeatingConic.backgroundGradient
               && repeatingConic.backgroundGradient->kind == radia::ui::GradientKind::Conic
    ...
    const radia::ui::Style solidOverride = stylesheet.resolve("panel", "", {"solid"}, 0);
...
template<> template<> void stylesheetObject::test<16>() {
    ...
    const radia::ui::Style one = stylesheet.resolve("panel", "", {"one"}, 0);
    ...
    const radia::ui::Style two = stylesheet.resolve("panel", "", {"two"}, 0);
    ...
    const radia::ui::Style longhandAfter = stylesheet.resolve("panel", "", {"longhand-after"}, 0);
    ...
    const radia::ui::Style shorthandAfter = stylesheet.resolve("panel", "", {"shorthand-after"}, 0);
    ...
    auto rejects = [&](const std::string& value) {
        const radia::ui::StyleSheetLoadResult result = stylesheet.loadRadia("panel { min-size: " + value + "; }", "min-size.radia");
    ...
    const radia::ui::Style preservedStyle = stylesheet.resolve("panel", "", {"one"}, 0);
...
template<> template<> void stylesheetObject::test<17>() {
    ...
    radia::ui::StyleSheet copy = original;
...
template<> template<> void stylesheetObject::test<18>() {
    radia::ui::StyleSheet stylesheet;
    const std::vector<radia::ui::ResourceLayer> layers{
    ...
    const radia::ui::Style resolved = stylesheet.resolve("panel", "", {}, 0);
...
template<> template<> void stylesheetObject::test<19>() {
    ...
    radia::ui::ResourceLayer layer{"theme/main.radia", kEntrypointStyles};
...
template<> template<> void stylesheetObject::test<20>() {
    ...
    auto layer = [](std::string source) {
        radia::ui::ResourceLayer result{"theme/main.radia", std::move(source)};
...
template<> template<> void stylesheetObject::test<22>() {
    ...
    const uint8_t invalid = static_cast<uint8_t>(radia::ui::WidgetState::Invalid);
...
template<> template<> void stylesheetObject::test<23>() {
    ...
    radia::ui::Text owner;
    const radia::ui::Style chord = stylesheet.resolveInline(owner, "kbd");
    const radia::ui::Style key = stylesheet.resolveInline(owner, "kbd", {"kbd"});
...
template<> template<> void stylesheetObject::test<24>() {
    radia::ui::StyleSheet stylesheet;
    radia::ui::ResourceLayer layer{"theme/main.radia", "@import \"branch-a.radia\"; @import \"branch-b.radia\";"};
    layer.entrypoint = "main.radia";
    layer.modules = {
    ...
    const radia::ui::StyleSheetLoadResult result = stylesheet.loadRadiaLayers({layer});
...
template<> template<> void stylesheetObject::test<25>() {
    ...
    ensure("border shorthand remains layout-affecting", stylesheet.stateAffectsLayout(radia::ui::WidgetState::Hovered));
}
} // namespace tut
