...
namespace rdui
{
    ...
    Color StyleSheet::Impl::parseColorValue(const std::string& raw, const Color& fallback) const
    {
        ...
        if (startsWith(lowered, "var(") && value.size() > 5 && value.back() == ')')
            return colorToken(trim(value.substr(4, value.size() - 5)), fallback);
    ...
    float StyleSheet::Impl::parseNumberValue(const std::string& raw, float fallback) const
    {
        ...
        const std::string lowered = lower(value);
        if (startsWith(lowered, "var(") && value.size() > 5 && value.back() == ')')
            return numberToken(trim(value.substr(4, value.size() - 5)), fallback);
        if (startsWith(lowered, "token(") && value.size() > 7 && value.back() == ')')
    ...
    std::optional<Gradient> StyleSheet::Impl::parseGradient(const std::string& raw) const
    {
        ...
        auto parseCenter = [&](const std::vector<std::string>& tokens, Vec2& center)
        {
            ...
            auto parsePercentage = [&](const std::string& token, float& percentage)
            {
                ...
                if (!parseFiniteFloat(token.substr(0, token.size() - 1), percentage)
                    || percentage < 0.f || percentage > 100.f) return false;
                percentage /= 100.f;
                return true;
            };
        ...
        if (!parsesAsColorStop(arguments.front()))
        {
            ...
            {
                if (prelude == "to top" || prelude == "to right" || prelude == "to bottom" || prelude == "to left"
                    || prelude == "to top right" || prelude == "to right top" || prelude == "to bottom right" || prelude == "to right bottom"
                    || prelude == "to bottom left" || prelude == "to left bottom" || prelude == "to top left" || prelude == "to left top")
                {
        ...
        while (run_start + 1 < gradient.stops.size())
        {
            ...
            const float step = (gradient.stops[run_end].position - gradient.stops[run_start].position)
                             / static_cast<float>(run_end - run_start);
        ...
        if (gradient.repeating
            && gradient.stops.back().position <= gradient.stops.front().position) return std::nullopt;
        return gradient;
    }
    ...
    std::optional<std::vector<Effect>> StyleSheet::Impl::parseEffects(const std::string& raw) const
    {
        ...
        for (const std::string& function : functions)
        {
            ...
            const std::vector<std::string> arguments = splitTopLevel(
                function.substr(prefix.size(), function.size() - prefix.size() - 1), ',');
            if (arguments.size() == 1)
            {
            ...
            {
                const std::vector<std::string> radii = tokenizeTopLevel(arguments[1]);
                if (!parseDirection(arguments[0], effect.angle_degrees) || radii.size() != 2
                    || !fixedRadius(radii[0], effect.start_radius)
                    || !fixedRadius(radii[1], effect.end_radius)) return std::nullopt;
            }
            else return std::nullopt;
