...
namespace rdui
{
    Path& Path::moveTo(float x, float y) { mCommands.push_back({PathVerb::MoveTo, {x, y}, {}, {}}); return *this; }
    Path& Path::lineTo(float x, float y) { mCommands.push_back({PathVerb::LineTo, {x, y}, {}, {}}); return *this; }
    Path& Path::quadTo(float cx, float cy, float x, float y) { mCommands.push_back({PathVerb::QuadTo, {cx, cy}, {x, y}, {}}); return *this; }
    ...
    Path& Path::close() { mCommands.push_back({PathVerb::Close, {}, {}, {}}); return *this; }
    ...
    namespace
    {
        ...
        {
            if (depth == 12 || distanceToLine(b, a, c) <= tolerance) { out.push_back(c); return; }
    ...
    PathCompileResult compileSvgPathData(const std::string& data, const std::string& source, std::size_t line)
    {
        ...
        while (*cursor)
        {
            ...
            if (std::isalpha(static_cast<unsigned char>(*cursor)))
            {
                ...
                if (normalized != 'M' && normalized != 'L' && normalized != 'H' && normalized != 'V'
                    && normalized != 'Q' && normalized != 'C' && normalized != 'Z')
                {
            ...
            {
                float cx = 0.f, cy = 0.f;
                if (!number(cursor, cx, !command_was_read) || !number(cursor, cy, true)
                    || !number(cursor, x, true) || !number(cursor, y, true))
                {
            ...
            else if (op == 'C')
            {
                ...
                if (!number(cursor, c0x, !command_was_read) || !number(cursor, c0y, true)
                    || !number(cursor, c1x, true) || !number(cursor, c1y, true)
                    || !number(cursor, x, true) || !number(cursor, y, true))
                {
