...
namespace rdui
{
    ...
    LocalizationLoadResult LocalizationCatalog::Impl::load(
        const std::vector<ResourceLayer>& layers)
    {
        ...
        for (ParsedLocale& parsed : base.locales)
        {
            ...
            if (localeIdentity(parsed.id) == default_identity && parsed.fallback)
                result.error("localization.default.fallback_forbidden",
                             "The default locale cannot declare a fallback.",
        ...
        for (std::size_t layer_index = 1; layer_index < layers.size(); ++layer_index)
        {
            ...
            for (ParsedLocale& parsed : patch.locales)
            {
                ...
                if (existing == locale_indices.end())
                {
                    if (!parsed.name)
                    {
                        result.error("localization.layer.locale.name_missing",
                                     "A locale introduced by a layer requires name: " + parsed.id + ".",
                                     parsed.source, parsed.line);
                        continue;
                    }
    ...
    void LocalizationCatalog::Impl::compile(LocalizationLoadResult& result)
    {
        ...
        std::sort(locales.begin(), locales.end(),
                  [](const LocaleRecord& left, const LocaleRecord& right)
                  {
                      return localeIdentity(left.info.id) < localeIdentity(right.info.id);
                  });
        ...
        for (std::size_t start = 0; start < locales.size(); ++start)
        {
            ...
            while (true)
            {
                ...
                {
                    result.error("localization.locale.fallback_cycle",
                                 "Locale fallback cycle contains " + locales[current].info.id + ".");
                    break;
                }
            ...
            for (const auto& [key, value] : locale.strings)
            {
                if (!defaults.contains(key))
                    result.error("localization.default.string_missing",
                                 "String '" + key + "' exists outside the default locale " + default_locale + ".",
        ...
        for (const auto& [key, default_value] : defaults)
        {
            ...
            const auto inspect_default = [&](const StringTemplate& value)
            {
                ...
                if (!binding_signature_set)
                {
                    ...
                    result.error("localization.string.bindings_mismatch",
                                 "All default variants must use the same kbd bindings: " + key + ".",
            ...
            for (const LocaleRecord& locale : locales)
            {
                ...
                const auto inspect_translation = [&](const StringTemplate& value)
                {
                    if (value.bindings != contract.bindings)
                        result.error("localization.string.bindings_mismatch",
                                     "Translation must preserve the default kbd bindings: " + key + ".",
                                     localized.source, localized.line);
                    if (localeIdentity(locale.info.id) == localeIdentity(default_locale)) return;
                    for (const std::string& argument : value.arguments) if (!contract.arguments.contains(argument)) introduced_arguments.insert(argument);
                };
            ...
            if (!introduced_arguments.empty())
            {
                if (!contract.plural || introduced_arguments.size() != 1)
                    result.error("localization.string.argument_unknown",
                                 "Translations introduce arguments absent from the default String: " + key + ".",
                                 default_value.source, default_value.line);
