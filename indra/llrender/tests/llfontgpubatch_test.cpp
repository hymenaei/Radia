...
namespace tut
{
    ...
    // position == pen + texcoord * scale (which holds even with the
    // half-pixel pre-dilation folded in), carrying the right glyph_loc and
    // color, and spanning at least the glyph's design-unit extents (the
    // dilation makes the quad slightly larger than the tight bbox).
    template<> template<>
    void llfontgpubatch_object::test<1>()
    {
        ...
        for (S32 i = 0; i < 6; ++i)
        {
            ...
            ensure("pos.x uses uniform scale",
                   approx(p[0], pen_x + uv[i].mV[0] * scale));
            ensure("pos.y uses uniform scale",
                   approx(p[1], pen_y + uv[i].mV[1] * scale));
    ...
    // (2) The COLOR and color-as-mask bits are preserved verbatim in every
    // vertex's glyph_loc. buildGlyphQuad is mode-agnostic and just forwards
    // the value.
    template<> template<>
    void llfontgpubatch_object::test<2>()
    {
        ...
        auto batch = LLFontGpuGlyphCache::beginBatch();
        const LLFontGpuGlyphCache::GlyphLoc loc =
            cache.getGlyph(batch, gidFor(tf.face, (hb_codepoint_t)'A'));
        ensure("'A' drawable", loc.drawable());
        ...
        for (S32 i = 0; i < 6; ++i)
        {
            ...
            ensure("color mask bit set",
                   (gl[i] & LLVertexBuffer::GLYPH_LOC_COLOR_AS_MASK) != 0u);
