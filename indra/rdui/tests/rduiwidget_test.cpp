...
template<> template<> void rduiwidget_object::test<2>() {
    rdui::Button button;
    int activations = 0;
...
template<> template<> void rduiwidget_object::test<3>() {
    rdui::Panel root;
...
template<> template<> void rduiwidget_object::test<4>() {
...
template<> template<> void rduiwidget_object::test<5>() {
    rdui::System system;
...
locales:
  ...
  ar:
    ...
    resources.add("skin.radia",
                  "panel { opacity: .5; effect: background-blur(3px), layer-blur(to right, 0px 0%, 4px 100%); }"
    ...
    ensure_equals("Text Host consumes resolved RTL alignment before painting the run", static_cast<int>(text_command->style.text_align),
    ...
    ensure("text is emitted before the later icon", text_command < icon_command);
    ensure("successful Surface paint clears invalidation", !surface->needsPaint());
}

template<> template<> void rduiwidget_object::test<6>() {
    ...
    ensure_equals("text runs preserve the authored content vertical alignment", static_cast<int>(commands[1].style.vertical_align),
    ...
    ensure_equals("B content strengthens the run style", commands[2].style.font_weight, static_cast<U16>(700));
    ...
    ensure_equals("letter spacing contributes across inline run boundaries", tracked.intrinsicSize(rdui::StyleSheet(), tracked_style, metrics).x,
                  15.f);
    ...
    ensure_equals("fixed metrics count grapheme rather than codepoint gaps",
                  tracked_combining.intrinsicSize(rdui::StyleSheet(), tracked_style, metrics).x, 17.f);
    rdui::RecordingPaintContext tracked_combining_recording(metrics);
    tracked_combining.paint(tracked_combining_recording, tracked_style, 1.f);
    ensure_equals("letter spacing does not split a cross-style grapheme", tracked_combining_recording.commands()[2].rect.x, 5.f);
    ...
    rtl_nodes.push_back(rdui::InlineContentNode::container(rdui::InlineContentKind::B, {rdui::InlineContentNode::text("الثاني")}));
    rdui::Text rtl_text;
    ...
    mixed_nodes.push_back(rdui::InlineContentNode::container(rdui::InlineContentKind::B, {rdui::InlineContentNode::text(" الثاني")}));
    rdui::Text mixed;
    ...
    ensure_equals("mixed bidi content splits at embedding and style boundaries", mixed_rtl.count(rdui::PaintCommandKind::Text), 3U);
    ensure("rightmost logical style run is painted first in visual order", mixed_rtl.commands()[1].style.font_weight == 700);
    ensure_equals("embedded LTR segment keeps its own visual run", mixed_rtl.commands()[2].value, std::string("123"));
    ensure("leading logical RTL segment is painted last", mixed_rtl.commands()[3].style.font_weight == 400);
    ...
    ensure("S content marks only its text run for strike-through", struck_recording.commands()[1].style.font_strike);
}

template<> template<> void rduiwidget_object::test<7>() {
    rdui::StyleSheet stylesheet;
    ensure("Kbd paint styles compile",
           stylesheet
               .loadRadia("text { font-size: 10px; line-height: 10px; } "
    ...
    for (const rdui::PaintCommand& command : recording.commands()) {
        ...
        if (command.kind == rdui::PaintCommandKind::Box && command.style.background_color.a > 0.f)
    ...
    ensure("outer and nested Kbd selectors produce distinct surfaces", box_colors.front().r != box_colors.back().r);
}

template<> template<> void rduiwidget_object::test<8>() {
    const rdui::FixedTextMetrics metrics(.5f, .5f);
    rdui::Style style;
    ensure_equals("text wraps by default like CSS", static_cast<int>(style.text_wrap), static_cast<int>(rdui::TextWrap::Wrap));
    ...
    ensure_equals("a shaped line that fits is not rewrapped after token measurement", natural_wrap.count(rdui::PaintCommandKind::Text), 1U);
    ensure_equals("the fitting shaped line remains intact", natural_wrap.commands()[1].value, std::string("RDUI Demo"));
    ...
    ensure_equals("center ellipsis preserves a balanced prefix and suffix", centered_text, std::string("abc\xE2\x80\xA6hij"));
    ...
    ensure_equals("end ellipsis preserves the visual start", ended_text, std::string("abcdef\xE2\x80\xA6"));
    ...
    ensure_equals("RTL end ellipsis paints at the visual left edge", rtl_ended.commands()[1].value, std::string("\xE2\x80\xA6"));
    ensure_equals("RTL end ellipsis retains the logical prefix", rtl_ended.commands()[2].value, std::string("ابتثجح"));
    style.direction = rdui::LayoutDirection::LeftToRight;

    rdui::Text combining("a\xCC\x81"
                         "bcdef");
    ...
    for (const rdui::PaintCommand& command : combining_ended.commands())
        if (command.kind == rdui::PaintCommandKind::Text) combining_text += command.value;
    ensure_equals("end ellipsis does not split a combining sequence", combining_text,
                  std::string("a\xCC\x81"
                              "b\xE2\x80\xA6"));
    ...
    rdui::RecordingPaintContext styled_combining_ended(metrics);
    styled_combining.paint(styled_combining_ended, style, 1.f);
    std::string styled_combining_text;
    for (const rdui::PaintCommand& command : styled_combining_ended.commands())
        if (command.kind == rdui::PaintCommandKind::Text) styled_combining_text += command.value;
    ensure_equals("ellipsis does not split a grapheme across inline styles", styled_combining_text, std::string("\xE2\x80\xA6"));

    rdui::Text family("\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9"
    ...
    for (const rdui::PaintCommand& command : family_ended.commands())
        if (command.kind == rdui::PaintCommandKind::Text) family_text += command.value;
    ensure_equals("end ellipsis does not split an emoji ZWJ sequence", family_text,
                  std::string("\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9"
                              "\xE2\x80\x8D\xF0\x9F\x91\xA7"
                              "A\xE2\x80\xA6"));
    ...
    ensure_equals("clip preserves the full run for the overflow scissor", clipped_recording.commands()[1].value, std::string("abc"));
    ...
    ensure_equals("a zero-width constraint preserves word wrap opportunities", zero_width_wrapping.count(rdui::PaintCommandKind::Text), 2U);
    ...
    ensure_equals("wrapping shapes each visual line instead of separate word tokens", shaped_wrapping.count(rdui::PaintCommandKind::Text), 2U);
    ensure_equals("the first visual line retains its internal space", shaped_wrapping.commands()[1].value, std::string("alpha beta"));
    ensure_equals("the following word starts the second visual line", shaped_wrapping.commands()[2].value, std::string("gamma"));

    rdui::Text cjk("你好世界");
    cjk.setRect({0.f, 0.f, 10.f, 20.f});
    rdui::RecordingPaintContext cjk_wrapping(metrics);
    cjk.paint(cjk_wrapping, style, 1.f);
    ensure_equals("Unicode line-break opportunities wrap CJK text", cjk_wrapping.count(rdui::PaintCommandKind::Text), 2U);
    ...
    ensure_equals("inline styling does not create a line-break opportunity", styled_word_wrapping.commands()[1].rect.y,
    ...
    for (const rdui::PaintCommand& command : asymmetric_center.commands())
        if (command.kind == rdui::PaintCommandKind::Text) asymmetric_text += command.value;
    ensure_equals("center ellipsis tries the suffix when the prefix is too wide", asymmetric_text,
