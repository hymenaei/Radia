...
struct detachedFloaterManagerData {
    struct PresentationState {
        ...
        rdui::Vec2 logicalSize{320.f, 240.f};
        rdui::Vec2 authoredSize{320.f, 240.f};
    ...
    public:
        ...
        std::optional<rdui::viewer::DetachedFloaterPresentationUpdate> open(
            const rdui::viewer::DetachedFloaterPresentationOpenRequest& request) override {
            mState.opened = true;
            ...
            return rdui::viewer::DetachedFloaterPresentationUpdate{
        ...
        void applyResize(const rdui::Rect& logicalRect) override { mState.logicalSize = {logicalRect.w, logicalRect.h}; }
        rdui::viewer::DetachedFloaterPresentationUpdate update() override {
            ++mState.ticks;
        ...
        std::optional<rdui::Rect> prepareReplacement(rdui::Floater& replacement) override {
            mState.replacementPrepared = true;
            if (!mState.replacementSucceeds) return std::nullopt;
            return rdui::Rect{0.f, 0.f, mState.authoredSize.x, mState.authoredSize.y};
        }
        std::unique_ptr<rdui::Floater> releaseFloater() override { return std::move(mFloater); }
        std::unique_ptr<rdui::Floater> replaceFloater(std::unique_ptr<rdui::Floater> replacement,
                                                      const std::optional<rdui::Vec2>& logicalSize) override {
            std::unique_ptr<rdui::Floater> retired = std::move(mFloater);
    ...
    struct FakeEnvironment final : rdui::viewer::DetachedFloaterManager::DetachedFloaterEnvironment {
        ...
        rdui::Vec2 nativeBottomLeftInMain(const AuxiliaryWindowRect&) const override { return {15.f, 25.f}; }
        ...
        std::optional<AuxiliaryWindowPoint> releasePointerForDetach(const rdui::Vec2& position) override {
        ...
        : settings("detached-floater-manager"), persistence(settings, settings),
          manager(
              ...
              [this](std::unique_ptr<rdui::Floater> floater) {
                  if (shouldFailFactory) return rdui::viewer::DetachedFloaterPresentationResult::failure(std::move(floater));
                  return rdui::viewer::DetachedFloaterPresentationResult::success(
                      std::make_unique<FakePresentation>(std::move(floater), presentation));
              },
              environment,
              [this](const rdui::viewer::ComponentKey& componentKey, rdui::viewer::FloaterPlacement placement,
                     rdui::viewer::ComponentOpenState state) { persistence.savePlacement(componentKey, std::move(placement), state); }) {
    ...
    rdui::viewer::ComponentPersistence persistence;
    rdui::viewer::ComponentKey identity{"demo", {}};
...
template<> template<> void detachedFloaterManagerObject::test<3>() {
    ...
    const bool restored = manager.restoreDetachedPlacement(identity, floater, {100, 200, rdui::viewer::FloaterLogicalSize{320.f, 240.f}, false});
...
template<> template<> void detachedFloaterManagerObject::test<4>() {
    ...
    const auto mountedResult = manager.replace(floater, std::move(replacement), rdui::Vec2{400.f, 300.f});
    rdui::Floater* mounted = mountedResult.installed();
...
template<> template<> void detachedFloaterManagerObject::test<7>() {
    ...
    const auto mountedResult = manager.replace(floater, std::move(replacement), rdui::Vec2{292.f, 300.f});
    rdui::Floater* mounted = mountedResult.installed();
...
template<> template<> void detachedFloaterManagerObject::test<10>() {
    ...
    const rdui::viewer::ComponentKey secondIdentity{"second", {}};
...
template<> template<> void detachedFloaterManagerObject::test<16>() {
    ...
    auto replacement = std::make_unique<rdui::Floater>();
    const std::optional<rdui::Rect> authoredRect = manager.prepareReplacement(floater, *replacement);
...
template<> template<> void detachedFloaterManagerObject::test<18>() {
    ...
    manager.reattachAll(rdui::viewer::DetachedFloaterManager::ReattachMode::PreservePlacement);
