...
namespace rdui
{
    namespace
    {
        ...
        bool hasNonWhitespaceText(LLXMLNode* node)
        {
            ...
            for (const unsigned char character : node->getValue()) if (!std::isspace(character)) return true;
            return false;
        }
        ...
        void validateAttributes(LLXMLNode* node, std::initializer_list<const char*> allowed, SvgCompileResult& result, const std::string& source)
        {
            ...
            for (const auto& entry : node->mAttributes)
            {
                ...
                if (!allowedAttribute(name, allowed))
                    result.error("svg.attribute.unsupported",
                                 "Unsupported attribute '" + name + "' on <" + elementName(node) + ">.",
                                 source, lineOf(node));
            }
        }

        bool validatePresentationValue(LLXMLNode* node, const char* name, const char* expected,
                                       SvgCompileResult& result, const std::string& source)
        {
            std::string value;
            if (!attribute(node, name, value)) return true;
            if (value == expected) return true;
            result.error("svg.presentation.unsupported",
                         "Unsupported " + std::string(name) + " value '" + value + "'. Expected '" + expected + "'.",
                         source, lineOf(node));
            return false;
        }
    ...
    SvgCompileResult compileSvgIcon(const std::string& svg, const std::string& source)
    {
        ...
        validateAttributes(root.get(), {"xmlns", "width", "height", "viewBox", "fill", "stroke",
                                        "stroke-width", "stroke-linecap", "stroke-linejoin", "class"}, result, source);
        ...
        if (!attribute(root.get(), "viewBox", raw)) result.error("svg.view_box.missing", "SVG icon requires a viewBox.", source, lineOf(root.get()));
            ...
            result.error("svg.view_box.invalid", "SVG viewBox must contain four finite numbers with positive width and height.", source, lineOf(root.get()));
        ...
        for (const char* dimension : {"width", "height"})
        {
            if (attribute(root.get(), dimension, raw))
            {
                ...
                if (!parseSingleFloat(raw, value) || value <= 0.f)
                    result.error("svg.dimension.invalid", "SVG " + std::string(dimension) + " must be a positive finite number.", source, lineOf(root.get()));
            }
        }
        ...
        for (LLXMLNodePtr child = root->getFirstChild(); child.notNull(); child = child->getNextSibling())
        {
            ...
            else if (name == "circle")
            {
                ...
                if (hasNonWhitespaceText(child.get())) result.error("svg.text.unsupported", "SVG <circle> cannot contain text content.", source, lineOf(child.get()));
                float cx = 0.f;
                float cy = 0.f;
                float radius = 0.f;
                std::string cx_raw;
                ...
                if (!attribute(child.get(), "cx", cx_raw) || !parseSingleFloat(cx_raw, cx)
                    || !attribute(child.get(), "cy", cy_raw) || !parseSingleFloat(cy_raw, cy)
                    || !attribute(child.get(), "r", radius_raw) || !parseSingleFloat(radius_raw, radius)
                    || radius <= 0.f)
                {
                    result.error("svg.circle.invalid", "SVG <circle> requires finite cx/cy and a positive finite radius.",
                                 source, lineOf(child.get()));
                    continue;
                }
        ...
        if (candidate.paths.empty() && !result.hasErrors()) result.error("svg.shapes.empty", "SVG icon contains no supported shapes.", source, lineOf(root.get()));
    ...
    Path transformSvgPath(const Path& path, const Rect& view_box, const Rect& target)
    {
        ...
        auto map = [&](const Vec2& point)
        {
            return Vec2(target.x + (point.x - view_box.x) * sx, target.y + target.h - (point.y - view_box.y) * sy);
        };
