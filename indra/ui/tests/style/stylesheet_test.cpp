...
template<> template<> void stylesheetObject::test<1>() {
    ...
    ensure_approximately_equals("rgb color token with alpha inherits into button label", resolveWidgetStyle(stylesheet, label).textColor.a, .5f, 6);
    ensure_equals("button caption inherits button font size", resolveWidgetStyle(stylesheet, label).fontSize, 17.f);
    Label standalone("Standalone");
    ensure_equals("standalone Label keeps its own font size", resolveWidgetStyle(stylesheet, standalone).fontSize, 29.f);
    const StyleSheetLoadResult invalid = stylesheet.loadRadia("switch { background-color: ##invalid; }", "invalid.radia");
...
template<> template<> void stylesheetObject::test<3>() {
    ...
    ensure_equals("orphan focus-visible state does not match",
                  stylesheet.resolve("button", "", {}, static_cast<uint8_t>(WidgetState::FocusVisible)).borderWidth.top, 1.f);
}

template<> template<> void stylesheetObject::test<4>() {
    StyleSheet stylesheet;
    ensure("initial stylesheet loads", stylesheet.loadRadia("button { width: 12px; }").ok());
    const StyleSheetLoadResult failed = stylesheet.loadRadia("button { width: 99px; unknown-property: 1; }", "candidate.radia");
...
template<> template<> void stylesheetObject::test<5>() {
    StyleSheet stylesheet;
    const StyleSheetLoadResult malformed = stylesheet.loadRadia(":root { --bad: nonsense; }", "tokens.radia");
    ...
    const StyleSheetLoadResult missing = stylesheet.loadRadia("button { width: var(--missing); }", "missing-token.radia");
...
template<> template<> void stylesheetObject::test<6>() {
    ...
    ensure_equals("child uses owner class", resolveWidgetStyle(stylesheet, icon).width.pixels(), 10.f);
    WidgetCompilerAccess::setState(button, WidgetState::Hovered, true);
    ensure_equals("child uses owner state", resolveWidgetStyle(stylesheet, icon).width.pixels(), 18.f);
}
...
template<> template<> void stylesheetObject::test<7>() {
    ...
    ensure_equals("owner hover does not hover interactive part", resolveWidgetStyle(stylesheet, *floater.closeButton()).width.pixels(), 10.f);
    WidgetCompilerAccess::setState(*floater.closeButton(), WidgetState::Hovered, true);
    ensure_equals("interactive part uses its own hover state", resolveWidgetStyle(stylesheet, *floater.closeButton()).width.pixels(), 18.f);
}
...
template<> template<> void stylesheetObject::test<8>() {
    ...
    ensure_equals("pointer cursor is typed", static_cast<int>(stylesheet.resolve("button", "", {}, 0).cursor),
                  static_cast<int>(CursorStyle::Pointer));
    ...
    const StyleSheetLoadResult invalid = stylesheet.loadRadia("button { cursor: teleport; }", "cursor.radia");
...
template<> template<> void stylesheetObject::test<9>() {
    ...
    const StyleSheetLoadResult valid = stylesheet.loadRadia(kValidStyles);
    ...
    auto rejects = [&](const std::string& radia, const std::string& code) {
        const StyleSheetLoadResult result = stylesheet.loadRadia(radia, "contract.radia");
    ...
    const StyleSheetLoadResult deadStates =
...
template<> template<> void stylesheetObject::test<10>() {
    ...
    ensure_equals("font family inherits", static_cast<int>(inheritedStyle.fontFamily), static_cast<int>(FontFamily::Sans));
    ...
    ensure_equals("text alignment inherits", static_cast<int>(inheritedStyle.textAlign), static_cast<int>(TextAlign::Center));
    ...
    ensure_equals("cursor inherits", static_cast<int>(inheritedStyle.cursor), static_cast<int>(CursorStyle::Grab));
    ensure_equals("opacity remains local for paint composition", inheritedStyle.opacity, 1.f);
    ensure_equals("pointer event policy does not inherit", static_cast<int>(inheritedStyle.pointerEvents), static_cast<int>(PointerEvents::Default));
    ...
    ensure_equals("explicit child cursor overrides inheritance", static_cast<int>(overriddenStyle.cursor), static_cast<int>(CursorStyle::Default));
}
...
template<> template<> void stylesheetObject::test<11>() {
    ...
    ensure_equals("overflow shorthand sets horizontal axis", static_cast<int>(split.overflowX), static_cast<int>(Overflow::Hidden));
    ensure_equals("overflow shorthand accepts a distinct vertical axis", static_cast<int>(split.overflowY), static_cast<int>(Overflow::Visible));
    ...
    ensure_equals("overflow longhand overrides shorthand horizontal value", static_cast<int>(overridden.overflowX),
                  static_cast<int>(Overflow::Visible));
    ensure_equals("overflow longhand overrides shorthand vertical value", static_cast<int>(overridden.overflowY), static_cast<int>(Overflow::Hidden));

    const StyleSheetLoadResult invalid = stylesheet.loadRadia("panel { overflow: scroll; }", "overflow.radia");
...
template<> template<> void stylesheetObject::test<12>() {
    ...
    ensure_equals("expanded header retains content separator", resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 1.f);
    floater.setMinimized(true);
    ensure("minimize projects widget state", floater.hasState(WidgetState::Minimized));
    ensure_equals("minimized owner state removes header separator", resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 0.f);
    floater.setMinimized(false);
    ensure("restore clears widget state", !floater.hasState(WidgetState::Minimized));
    ensure_equals("restored header regains content separator", resolveWidgetStyle(stylesheet, *floater.header()).borderWidth.bottom, 1.f);
}
...
template<> template<> void stylesheetObject::test<15>() {
    ...
    ensure("linear gradient kind is typed", style.backgroundGradient->kind == GradientKind::Linear);
    ...
    ensure("first effect targets background", effects.effects[0].kind == EffectKind::BackgroundBlur);
    ...
    ensure("second effect targets layer", effects.effects[1].kind == EffectKind::LayerBlur);
    ...
    ensure("radial background is typed", radial.backgroundGradient && radial.backgroundGradient->kind == GradientKind::Radial);
    ensure("radial shape is typed", radial.backgroundGradient->radialShape == RadialGradientShape::Circle);
    ...
    ensure("conic border-color is typed", radial.borderGradient && radial.borderGradient->kind == GradientKind::Conic);
    ...
    const Style repeatingRadial = stylesheet.resolve("switch", "", {}, 0);
           ...
           repeatingRadial.backgroundGradient
               && repeatingRadial.backgroundGradient->kind == GradientKind::Radial
    ...
    const Style repeatingConic = stylesheet.resolve("floater", "", {}, 0);
           ...
           repeatingConic.backgroundGradient
               && repeatingConic.backgroundGradient->kind == GradientKind::Conic
...
template<> template<> void stylesheetObject::test<16>() {
    ...
    auto rejects = [&](const std::string& value) {
        const StyleSheetLoadResult result = stylesheet.loadRadia("panel { min-size: " + value + "; }", "min-size.radia");
...
template<> template<> void stylesheetObject::test<24>() {
    StyleSheet stylesheet;
    ResourceLayer layer{"theme/main.radia", "@import \"branch-a.radia\"; @import \"branch-b.radia\";"};
    layer.entrypoint = "main.radia";
    layer.modules = {
    ...
    const StyleSheetLoadResult result = stylesheet.loadRadiaLayers({layer});
...
template<> template<> void stylesheetObject::test<25>() {
    ...
    ensure("border shorthand remains layout-affecting", stylesheet.stateAffectsLayout(WidgetState::Hovered));
}
} // namespace tut
