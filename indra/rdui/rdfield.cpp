        ...
        return defineWidget<Field>(Field::ELEMENT)
                            ...
                            ViewBuildResult& result, const std::string& source)
            {
                ...
                for (const auto& child : field.children())
                {
                    ...
                    else result.error("view.field.child_unsupported",
                                      "Field accepts only Label, one Value Control, Hint, Error, and Flow Break.",
                                      source, element.source().begin.line, element.source().begin.column);
                }
                if (labels.size() != 1)
                    result.error("view.field.label_required", "Field requires exactly one direct Label.",
                                 source, element.source().begin.line, element.source().begin.column);
                if (controls.size() != 1)
                    result.error("view.field.control_required", "Field requires exactly one direct Value Control.",
                                 source, element.source().begin.line, element.source().begin.column);
