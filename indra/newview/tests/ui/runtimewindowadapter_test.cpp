...
struct runtimeWindowAdapterData {
    ...
    int clears = 0;
    radia::viewer::ui::RuntimeWindowAdapter adapter{window, auxiliaryWindows, [] { return radia::ui::Vec2{2.f, 2.f}; }, [] { return std::pair{800, 600}; },
                                               [this] { ++clears; }};
};
...
template<> template<> void runtimeWindowAdapterObject::test<3>() {
    const radia::ui::Vec2 scale{2.f, 2.f};
    const radia::ui::Vec2 native = radia::viewer::ui::detail::scaleLogicalPoint({10.f, 15.f}, scale);
    ensure_equals("logical x is scaled once", native.x, 20.f);
    ensure_equals("logical y is scaled once", native.y, 30.f);
    const radia::ui::Vec2 logical = radia::viewer::ui::detail::unscaleNativePoint(native, scale);
