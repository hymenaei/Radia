...
private:
    bool parseNodes(std::vector<TemplateNode>& nodes, const ContainerTag* expected_close) {
        while (mIndex < mTokens.size()) {
            const RichToken& current = mTokens[mIndex];
            if (current.kind == RichTokenKind::ContainerClose) {
                if (current.container == expected_close) {
                    ++mIndex;
                    return true;
                }
                mResult.error("localization.string.tag_invalid", "Unknown, malformed, or misplaced Rich String tag: " + current.value + ".",
                              mSourceName, tokenLine(current));
                return false;
            }

            ++mIndex;
            switch (current.kind) {
                case RichTokenKind::Text: appendText(nodes, current.value); break;
                case RichTokenKind::Argument:
                    mParsed.arguments.insert(current.value);
                    nodes.push_back({TemplateKind::Argument, current.value, {}});
                    break;
                case RichTokenKind::ContainerOpen: {
                    TemplateNode node{current.container->kind, {}, {}};
                    if (!parseNodes(node.children, current.container)) return false;
