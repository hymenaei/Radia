...
void ensureColor(const std::string& message, const std::string& value, const radia::ui::Color& expected) {
    const std::optional<radia::ui::Color> color = radia::ui::parseColor(value);
...
template<> template<> void colorObject::test<1>() {
    ...
    ensure("double hash rejected", !radia::ui::parseColor("##ff880080"));
}
...
template<> template<> void colorObject::test<2>() {
    ...
    ensure("rgba function rejected", !radia::ui::parseColor("rgba(255 128 0 / 50%)"));
}
...
template<> template<> void colorObject::test<3>() {
    ...
    ensure("hsla function rejected", !radia::ui::parseColor("hsla(120 100% 50% / .5)"));
}

template<> template<> void colorObject::test<4>() {
    ensure("bad hex rejected", !radia::ui::parseColor("#ggg"));
    ensure("mixed separators rejected", !radia::ui::parseColor("rgb(1, 2, 3 / .5)"));
    ensure("missing percent rejected", !radia::ui::parseColor("hsl(0 1 1)"));
    ensure("unknown function rejected", !radia::ui::parseColor("color(1 2 3)"));
    ensure("modern color commas rejected", !radia::ui::parseColor("lab(50%, 0, 0)"));
}
