...
std::unique_ptr<Floater> takeFloaterRoot(LayoutBuildResult layout, const std::string& rootError, const std::string& source,
    ...
    rdui::Widget* root = layout.root.get();
...
struct ComponentManager::Impl final {
    ...
    void evictClosed() {
        for (auto pending = pendingEvictions.begin(); pending != pendingEvictions.end();) {
            ...
            if (!retired) {
                LL_WARNS("rdui") << "Closed component could not be unmounted: " << pending->persistenceKey() << LL_ENDL;
                ++pending;
                continue;
            }
            if (retired.get() != root)
                LL_ERRS("rdui") << "Component host returned the wrong root while unmounting: " << pending->persistenceKey() << LL_ENDL;
...
bool ComponentManager::clearInstances() {
    ...
    if (!mImpl->host.clearAll(std::move(roots))) {
        LL_WARNS("rdui") << "Component host rejected account teardown; retaining the current component generation." << LL_ENDL;
        return false;
    }
