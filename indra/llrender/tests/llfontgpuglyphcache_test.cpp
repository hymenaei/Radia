...
namespace tut
{
    ...
    // (3) Overflow eviction: adding the second drawable glyph marks a deferred
    // reset rather than invalidating offsets halfway through the current draw.
    // At the next safe boundary the reset bumps the generation, and a re-fetch
    // lands the survivor at offset 0.
    template<> template<>
    void llfontgpuglyphcache_object::test<3>()
    {
        ...
        {
            auto batch = LLFontGpuGlyphCache::beginBatch();
            const LLFontGpuGlyphCache::GlyphLoc a =
                cache.getGlyph(batch, gid_A);
            ensure("'A' drawable", a.drawable());

            // Ceiling = exactly 'A's footprint, so the next glyph tips us over.
            cache.setMaxTexels(a.mTexelCount);
            const LLFontGpuGlyphCache::GlyphLoc b =
                cache.getGlyph(batch, gid_B);
            ensure("'B' drawable", b.drawable());
            ensure_equals("generation pinned for the complete batch",
                          cache.getGeneration(), gen_before);
            ensure_equals("both current-batch offsets remain cached",
                          cache.getGlyphCount(), 2U);
            ensure("second glyph follows the first in the live arena",
                   b.mTexelOffset >= a.mTexelOffset + a.mTexelCount);
        }

        // The cache owns the boundary protocol: beginning the next transaction
        // drains the old generation and performs its deferred reset.
        auto next_batch = LLFontGpuGlyphCache::beginBatch();
        ensure("next batch reset bumped generation",
               cache.getGeneration() > gen_before);
        const LLFontGpuGlyphCache::GlyphLoc b_after =
            cache.getGlyph(next_batch, gid_B);
        ensure_equals("survivor re-encodes at offset 0",
                      b_after.mTexelOffset, 0U);
        ensure_equals("stale per-face map was cleared lazily",
    ...
    void llfontgpuglyphcache_object::test<5>()
    {
        ...
        LLFontManager::initClass();
        {
            ...
            auto batch = LLFontGpuGlyphCache::beginBatch();
            ensure("face cache encodes 'A'",
                   cache->getGlyph(batch, (U32)gid).drawable());
        }
    ...
    void llfontgpuglyphcache_object::test<7>()
    {
        ...
        reg.init(tf.font);
        auto batch = LLFontGpuGlyphCache::beginBatch();
        const LLFontGpuGlyphCache::GlyphLoc loc_reg =
            reg.getGlyph(batch, gid_I);
        ...
        bold.init(bold_font);
        const LLFontGpuGlyphCache::GlyphLoc loc_bold =
            bold.getGlyph(batch, gid_I);
