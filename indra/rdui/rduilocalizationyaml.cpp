...
namespace rdui::localization_detail
{
    namespace
    {
        ...
        public:
            ...
            {
                reject(mark, "YAML aliases are forbidden.");
            }
            ...
            {
                reject(mark, "YAML anchors are forbidden.");
            }
        ...
        std::optional<std::string> canonicalLanguageTag(
        ...
        {
            static const std::unordered_set<std::string> categories = {"zero", "one", "two", "few", "many", "other",};
        ...
        private:
            std::optional<YAML::Node> loadRoot()
            {
                ...
                {
                    mResult.error("localization.yaml.utf8_invalid",
                                  "Localization YAML must be valid UTF-8.",
                                  mSourceName);
                    return std::nullopt;
                }
                ...
                try
                {
                    ...
                    if (!event_handler.valid())
                    {
                        ...
                        mResult.error("localization.yaml.feature_forbidden",
                                      event_handler.reason(), mSourceName,
                ...
                {
                    mResult.error("localization.yaml.invalid",
                                  "Could not parse Radia UI localization YAML: " + std::string(exception.what()),
                                  mSourceName,
                ...
                {
                    mResult.error("localization.root.invalid",
                                  "Localization YAML must contain exactly one mapping document.",
                                  mSourceName,
            ...
            std::optional<std::string> scalar(const YAML::Node& node, const std::string& field)
            {
                if (!node.IsScalar())
                {
                    mResult.error("localization.scalar.required",
                                  field + " must be a YAML string.", mSourceName,
                ...
                if (value.empty() || implicitNonString(node, value))
                {
                    mResult.error("localization.scalar.invalid",
                                  field + " must be a non-empty YAML string; quote values that resemble numbers, booleans, or null.",
                                  mSourceName, lineOf(node), columnOf(node));
                    return std::nullopt;
                }
                if (!validCatalogText(value))
                {
                    mResult.error("localization.string.unicode_invalid",
                                  field + " must be NFC-normalized Unicode without tabs or control characters.",
                                  mSourceName, lineOf(node), columnOf(node));
                    return std::nullopt;
                }
                return value;
            ...
            std::vector<MappingEntry> mapping(const YAML::Node& map, const std::string& field, const std::unordered_set<std::string>& allowed = {})
            {
                ...
                if (!map.IsMap())
                {
                    mResult.error("localization.map.required",
                                  field + " must be a YAML mapping.", mSourceName,
                                  lineOf(map), columnOf(map));
                    return result;
                }
                ...
                for (const auto& entry : map)
                {
                    ...
                    {
                        mResult.error("localization.yaml.key_duplicate",
                                      "Duplicate YAML key in " + field + ": " + *key + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                    if (!allowed.empty() && !allowed.contains(*key))
                    {
                        mResult.error("localization.field.unknown",
                                      "Unknown field in " + field + ": " + *key + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
            ...
            void parseRoot(const YAML::Node& root, ParsedCatalog& catalog)
            {
                ...
                if (!locales_seen)
                {
                    mResult.error("localization.locales.missing",
                                  "Localization YAML requires a locales mapping.",
                                  mSourceName);
                    return;
                }
            ...
            void parseDefaultLocale(const YAML::Node& node, ParsedCatalog& catalog)
            {
                if (!mBase)
                {
                    mResult.error("localization.layer.default_forbidden",
                                  "Localization override layers must omit defaultLocale.",
                                  mSourceName, lineOf(node), columnOf(node));
                    return;
                }
                ...
                if (!canonical || *canonical != *id)
                    mResult.error("localization.default.invalid",
                                  "defaultLocale must be a canonical BCP 47 Locale ID: " + *id + ".",
                                  mSourceName, lineOf(node), columnOf(node));
            ...
            void parseLocales(const YAML::Node& node, ParsedCatalog& catalog)
            {
                if (!node.IsMap())
                {
                    mResult.error("localization.locales.invalid",
                                  "locales must be a YAML mapping.", mSourceName,
                                  lineOf(node), columnOf(node));
                    return;
                }
                ...
                for (const auto& entry : node)
                {
                    ...
                    {
                        mResult.error("localization.locale.id_invalid",
                                      "Locale ID must be a canonical BCP 47 tag: " + *authored_id + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                    ...
                    {
                        mResult.error("localization.locale.duplicate",
                                      "Duplicate localization locale: " + *canonical + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                ...
                if (mBase && catalog.locales.empty())
                    mResult.error("localization.locales.empty",
                                  "Base localization YAML defines no locales.",
                                  mSourceName);
            }
            ...
            ParsedLocale parseLocale(const std::string& id, const YAML::Node& id_node, const YAML::Node& node)
            {
                ...
                if (!locale.strings_present)
                    mResult.error("localization.locale.strings_missing",
                                  "Every locale entry requires a strings mapping: " + locale.id + ".",
                                  mSourceName, locale.line);
                if (mBase && !locale.name)
                    mResult.error("localization.locale.name_missing",
                                  "Base locale has no display name: " + locale.id + ".",
                                  mSourceName, locale.line);
                return locale;
            }
            ...
            void parseLocaleName(const YAML::Node& node, ParsedLocale& locale)
            {
                ...
                const bool rich = name->find('\n') != std::string::npos
                                                   ...
                                                   || std::any_of(parsed.nodes.begin(), parsed.nodes.end(),
                                                                  [](const TemplateNode& value)
                                                                  {
                                                                      return value.kind != TemplateKind::Text;
                                                                  });
                if (rich || parsed_result.hasErrors())
                    mResult.error("localization.locale.name_invalid",
                                  "Locale name must be a single-line plain string without markup or placeholders.",
                                  mSourceName, lineOf(node), columnOf(node));
            ...
            void parseLocaleDirection(const YAML::Node& node, ParsedLocale& locale)
            {
                ...
                if (*direction == "ltr") locale.direction = LayoutDirection::LeftToRight;
                    ...
                    mResult.error("localization.locale.direction_invalid",
                                  "Locale direction must be exactly 'ltr' or 'rtl'.",
                                  mSourceName, lineOf(node), columnOf(node));
            }
            ...
            void parseLocaleFallback(const YAML::Node& node, ParsedLocale& locale)
            {
                ...
                if (!canonical || *canonical != *fallback)
                    mResult.error("localization.locale.fallback_invalid",
                                  "Locale fallback must be a canonical BCP 47 Locale ID: " + *fallback + ".",
                                  mSourceName, lineOf(node), columnOf(node));
            ...
            void parseStrings(const YAML::Node& node, ParsedLocale& locale)
            {
                ...
                if (!node.IsMap())
                {
                    mResult.error("localization.strings.invalid",
                                  "Locale strings must be a flat YAML mapping.",
                                  mSourceName, lineOf(node), columnOf(node));
                    return;
                }
                ...
                for (const auto& entry : node)
                {
                    ...
                    {
                        mResult.error("localization.string.key_invalid",
                                      "String Key must use lowerCamelCase segments: " + *key + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                    ...
                    {
                        mResult.error("localization.string.duplicate",
                                      "Duplicate String Key in locale " + locale.id + ": " + *key + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
            ...
            std::optional<StringValue> parseStringValue(const std::string& key, const YAML::Node& key_node, const YAML::Node& node)
            {
                ...
                mResult.error("localization.string.value_invalid",
                              "Localized value must be a non-empty YAML string or Plural Category map.",
                              mSourceName, lineOf(node), columnOf(node));
                return std::nullopt;
            }

            PluralTemplates parsePluralVariants(
                const std::string& key, const YAML::Node& node)
            {
                ...
                for (const auto& entry : node)
                {
                    ...
                    {
                        mResult.error("localization.plural.category_duplicate",
                                      "Duplicate Plural Category " + *category + " for " + key + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                    ...
                    {
                        mResult.error("localization.plural.category_invalid",
                                      "Unknown CLDR Plural Category: " + *category + ".",
                                      mSourceName, lineOf(entry.first),
                                      columnOf(entry.first));
                        continue;
                    }
                    ...
                    if (!text) continue;
                    StringTemplate parsed;
                    if (parseRichString(*text, parsed, mResult, mSourceName,lineOf(entry.second)))
                        plurals.emplace(*category, std::move(parsed));
                }

                if (!plurals.contains("other"))
                    mResult.error("localization.plural.other_missing",
                                  "Plural String requires an other variant: " + key + ".",
                                  mSourceName, lineOf(node), columnOf(node));
                return plurals;
            }
