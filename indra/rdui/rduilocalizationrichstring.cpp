...
namespace rdui::localization_detail
{
    namespace
    {
        ...
        enum class RichTokenKind : std::uint8_t
        {
            Text,
            Argument,
            ContainerOpen,
        ...
        std::optional<ScannedToken> scanRecognizedTag(
            const std::string& source, std::size_t offset)
        {
            ...
            if (source.compare(offset, BR.size(), BR) == 0)
                return ScannedToken{{RichTokenKind::Br, {}, offset, nullptr}, BR.size()};
                    ...
                    while (offset < mSource.size())
                    {
                        ...
                        if (mSource[offset] == '\\')
                        {
                            ...
                            mResult.error("localization.string.escape_invalid",
                                          offset + 1 >= mSource.size()
                                              ? "A trailing backslash is not a valid Rich String escape."
                        ...
                        if (mSource[offset] == '<')
                        {
                            ...
                            if (tag_shaped && end != std::string::npos)
                            {
                                mResult.error("localization.string.tag_invalid",
                                              "Unknown, malformed, or misplaced Rich String tag: " + mSource.substr(offset, end - offset + 1)+ ".",
                                              mSourceName,
        ...
        class RichStringParser
        {
            public:
                RichStringParser(
                    const std::vector<RichToken>& tokens,
                    const std::string& source,
                ...
                bool parse()
                {
                    return parseNodes(mParsed.nodes, nullptr);
                }
                ...
                bool parseNodes(std::vector<TemplateNode>& nodes, const ContainerTag* expected_close)
                {
                    while (mIndex < mTokens.size())
                    {
                        ...
                        if (token.kind == RichTokenKind::ContainerClose)
                        {
                            ...
                            mResult.error("localization.string.tag_invalid",
                                          "Unknown, malformed, or misplaced Rich String tag: " + token.value + ".",
                        ...
                        switch (token.kind)
                        {
                            ...
                            case RichTokenKind::ContainerOpen:
                            {
                                ...
                                if (!parseNodes(node.children, token.container))
                                    return false;
                            ...
                            case RichTokenKind::Kbd:
                                ...
                                break;
                            case RichTokenKind::Br:
                                nodes.push_back({TemplateKind::Br, {}, {}});
                                break;
                            case RichTokenKind::ContainerClose:
                                break;
                        }
                    }

                    if (!expected_close) return true;
                    mResult.error("localization.string.tag_unclosed",
                                  "Rich String is missing closing tag " + std::string(expected_close->close) + ".",
    ...
    bool parseRichString(const std::string& source, StringTemplate& parsed, LocalizationLoadResult& result, const std::string& source_name, std::size_t line)
    {
        parsed = {};
