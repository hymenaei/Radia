/**
 * @file rduiinlinecontentcompiler.cpp
 * @brief
 *
...
TextSource compileInlineContent(const std::vector<LayoutContent>& content_items, const std::string& host,
                                const std::vector<InlineContentKind>& accepted, ViewBuildResult& result, const std::string& source,
    ...
    const auto inlineText = [&](const std::string& authored, std::size_t line) {
        ...
        if (context) {
            ...
            const bool has_leading_space = !authored.empty() && std::isspace(static_cast<unsigned char>(authored.front()));
            const bool has_trailing_space = !authored.empty() && std::isspace(static_cast<unsigned char>(authored.back()));
            if (key.empty()) {
    ...
    buildInline = [&](const std::vector<LayoutContent>& content) {
        ...
        for (const LayoutContent& item : content) {
            ...
            if (!inlineContentKind(inline_node.name, kind)) {
                result.error("view.inline.element_unknown", "Unsupported inline element in <" + host + ">: " + inline_node.name + ".", source,
                             inline_node.source.begin.line, inline_node.source.begin.column);
                continue;
            }
            if (!accepts(kind)) {
                result.error("view.inline.unsupported",
                             "Inline <" + std::string(inlineContentElement(kind)) + "> is not supported in <" + host + ">.", source,
                             inline_node.source.begin.line, inline_node.source.begin.column);
                continue;
            }
            if (kind == InlineContentKind::Link) {
                result.error("view.inline.not_implemented", "Inline <" + std::string(inlineContentElement(kind)) + "> is not implemented yet.",
            ...
            if (kind == InlineContentKind::Kbd) {
                ...
                for (const auto& [name, attribute] : inline_node.attributes)
                    if (name != "binding")
                        result.error("view.inline.attribute_unknown", "Unknown attribute on inline <kbd>: " + attribute.authored_name + ".", source,
                                     attribute.source.begin.line, attribute.source.begin.column);
                if (binding == inline_node.attributes.end())
                    result.error("view.inline.kbd.binding_required", "Inline <kbd> requires a binding attribute.", source,
                                 inline_node.source.begin.line, inline_node.source.begin.column);
                else if (!isLocalIdentifier(binding->second.value))
                    result.error("view.inline.kbd.binding_invalid", "Inline <kbd> binding must be a lowercase kebab-case command id.", source,
                                 binding->second.source.begin.line, binding->second.source.begin.column);
                if (hasAuthoredContent(inline_node))
                    result.error("view.inline.children_unsupported", "Inline <kbd> cannot contain authored content.", source,
                                 inline_node.source.begin.line, inline_node.source.begin.column);
            ...
            for (const auto& [name, attribute] : inline_node.attributes)
                result.error("view.inline.attribute_unknown",
                             "Unknown attribute on inline <" + std::string(inlineContentElement(kind)) + ">: " + attribute.authored_name + ".",
                             source, attribute.source.begin.line, attribute.source.begin.column);

            if (kind == InlineContentKind::Br) {
                if (hasAuthoredContent(inline_node))
                    result.error("view.inline.children_unsupported", "Inline <br> cannot contain content.", source, inline_node.source.begin.line,
                                 inline_node.source.begin.column);
