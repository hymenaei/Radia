...
namespace rdui
{
    float TextMetrics::usedLetterSpacing(const Style& style) const
    {
        ...
        return style.letter_spacing.resolve(
            measureText(" ", reference_style).x);
    }
    ...
    Vec2 FixedTextMetrics::measureText(const std::string& text, const Style& style) const
    {
        ...
        const std::vector<std::size_t> graphemes =
            detail::graphemeBoundaries(wide);
        const std::size_t grapheme_count =
            graphemes.empty() ? characters : graphemes.size() - 1;
        const float letter_spacing_width = grapheme_count > 1
            ? letter_spacing * static_cast<float>(grapheme_count - 1)
            : 0.f;
        const std::size_t word_separators = static_cast<std::size_t>(std::count_if(
            wide.begin(), wide.end(), [](llwchar character)
            {
        ...
        return {
            std::ceil(std::max(
                0.f,
                static_cast<float>(characters) * character_width
                    + letter_spacing_width + word_spacing_width)),
            line_height,
        };
    }
    ...
    {
        const float weight_mix = std::clamp(
            (static_cast<float>(style.font_weight) - 400.f) / 300.f,
            0.f, 1.f);
        const float factor = mRegularWidthFactor
                           + (mBoldWidthFactor - mRegularWidthFactor)
                           * weight_mix;
