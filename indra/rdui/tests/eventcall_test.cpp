...
template<> template<> void eventCallObject::test<1>() {
    const rdui::EventCallParseResult parsed = rdui::parseEventCall("press()");
...
template<> template<> void eventCallObject::test<2>() {
    const rdui::EventCallParseResult parsed = rdui::parseEventCall("selectLocale(+1)");
...
template<> template<> void eventCallObject::test<3>() {
    const rdui::EventCallParseResult parsed = rdui::parseEventCall("inspect('settings', true, false, this, event)");
    ...
    ensure("this has a distinct type", std::holds_alternative<rdui::SourceWidgetArgument>(arguments[3]));
    ensure("event has a distinct type", std::holds_alternative<rdui::CurrentEventArgument>(arguments[4]));
}

template<> template<> void eventCallObject::test<4>() {
    const rdui::EventCallParseResult parsed = rdui::parseEventCall("  open ( 'settings' , true )  ");
...
template<> template<> void eventCallObject::test<5>() {
    ...
    ensure_equals("bare Handler reports required call", static_cast<int>(parsed.error), static_cast<int>(rdui::EventCallParseError::CallRequired));
}

template<> template<> void eventCallObject::test<6>() {
    for (const char* source : {"Save()", "save-profile()", "save_profile()", "save.profile()"}) {
        const rdui::EventCallParseResult parsed = rdui::parseEventCall(source);
        ...
        ensure_equals("invalid Handler name has a stable error", static_cast<int>(parsed.error),
                      static_cast<int>(rdui::EventCallParseError::NameInvalid));
    }
}
...
template<> template<> void eventCallObject::test<9>() {
    const rdui::EventCallParseResult parsed = rdui::parseEventCall("select(9223372036854775808)");
    ensure("out-of-range integer is rejected", !parsed.ok());
    ensure_equals("integer range has a stable error", static_cast<int>(parsed.error), static_cast<int>(rdui::EventCallParseError::IntegerOutOfRange));
}
