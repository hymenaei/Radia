...
namespace tut {
class FloaterDelegateProbe final : public radia::ui::SurfaceFloaterDelegate {
public:
    bool canDetachFloater(const radia::ui::Surface&, const radia::ui::Floater&) const override { return allowDetach; }
    void floaterClosed(radia::ui::Surface&, radia::ui::Floater&) override { ++closes; }
    void floaterMinimizedChanged(radia::ui::Surface&, radia::ui::Floater&, bool) override { ++minimizeChanges; }
    void floaterMoved(radia::ui::Surface&, radia::ui::Floater&) override { ++moves; }
    void floaterMoveEnded(radia::ui::Surface&, radia::ui::Floater&) override { ++moveCompletions; }
    bool beginNativeFloaterResize(radia::ui::Surface& surface, radia::ui::Floater& floater) override {
        ++resizeStarts;
        if (unmountOnNativeResize) unmountedFloater = surface.unmountFloater(floater);
        return nativeResize;
    }
    void floaterResized(radia::ui::Surface&, radia::ui::Floater&, bool complete) override {
        ++resizeChanges;
        if (complete) ++resizeCompletions;
    }
    void floaterDetachRequested(radia::ui::Surface&, radia::ui::Floater&, const radia::ui::Vec2& desiredPosition, const radia::ui::Vec2&) override {
        ++detachRequests;
    ...
    std::unique_ptr<radia::ui::Floater> unmountedFloater;
    radia::ui::Vec2 requestedPosition;
};
...
template<> template<> void floatersObject::test<1>() {
    ...
    surface.pointerDown({{30.f, 110.f}, radia::ui::PointerButton::Left});
    surface.pointerMove({{-99.f, 110.f}, radia::ui::PointerButton::Left});
    ensure_equals("99 logical pixels beyond the Surface remain attached", delegate.detachRequests, 0);
    surface.pointerMove({{-100.f, 110.f}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<2>() {
    ...
    surface.updateLayout();
    const radia::ui::Vec2 dragStart{target->rect().left() + 2.f, target->rect().top() - 15.f};
    ensure("minimized header starts drag", surface.pointerDown({dragStart, radia::ui::PointerButton::Left}));
    surface.pointerMove({{199.f, dragStart.y}, radia::ui::PointerButton::Left});
    surface.pointerUp({{199.f, dragStart.y}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<3>() {
    ...
    std::unique_ptr<radia::ui::Floater> transferred = first.unmountFloater(*target);
...
template<> template<> void floatersObject::test<5>() {
    ...
    surface.mountFloater(std::move(floater));
    const std::optional<radia::ui::Rect> prepared = surface.prepareFloater(*target);
    ...
    ensure("right border has intrinsic cursor", surface.cursor() == radia::ui::CursorStyle::EastWestResize);
    ensure("right border starts capture",
           surface.pointerDown({{target->rect().right() - 1.f, target->rect().bottom() + 30.f}, radia::ui::PointerButton::Left}));
    surface.pointerMove({{250.f, target->rect().bottom() + 30.f}, radia::ui::PointerButton::Left});
    ...
    surface.pointerUp({{250.f, target->rect().bottom() + 30.f}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<6>() {
    ...
    ensure("disabled resizing exposes no intrinsic cursor", surface.cursor() == radia::ui::CursorStyle::Default);
    ...
    ensure("minimized Floater exposes no resize cursor", surface.cursor() == radia::ui::CursorStyle::Default);
}
...
template<> template<> void floatersObject::test<7>() {
    ...
    const radia::ui::Vec2 leftEdge{target->rect().left() + 1.f, target->rect().bottom() + 30.f};
    surface.pointerDown({leftEdge, radia::ui::PointerButton::Left});
    surface.pointerMove({{target->rect().right() + 500.f, leftEdge.y}, radia::ui::PointerButton::Left});
    surface.pointerUp({{target->rect().right() + 500.f, leftEdge.y}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<8>() {
    ...
    ensure("native-hosted resize starts", surface.pointerDown({{99.f, 40.f}, radia::ui::PointerButton::Left}));
    surface.pointerMove({{139.f, 40.f}, radia::ui::PointerButton::Left});
    ensure_equals("native-hosted logical geometry may grow beyond its old viewport", target->rect().w, 140.f);
    surface.pointerUp({{139.f, 40.f}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<9>() {
    ...
    surface.mountFloater(std::move(floater), radia::ui::SurfaceLayer::Modal);

    ensure("modal resize starts before modal child routing", surface.pointerDown({{119.f, 60.f}, radia::ui::PointerButton::Left}));
    surface.pointerMove({{159.f, 60.f}, radia::ui::PointerButton::Left});
    surface.pointerUp({{159.f, 60.f}, radia::ui::PointerButton::Left});
...
template<> template<> void floatersObject::test<10>() {
    ...
    const radia::ui::Rect firstOuter = surface.prepareFloater(*floater).value();
    const radia::ui::Vec2 firstContent = floater->authoredContentSize();
    floater->addChild(std::make_unique<radia::ui::Label>("second"));
    const radia::ui::Rect secondOuter = surface.prepareFloater(*floater).value();
    const radia::ui::Vec2 secondContent = floater->authoredContentSize();
...
template<> template<> void floatersObject::test<11>() {
    ...
    const radia::ui::Rect rect = surface.prepareFloater(floater).value();
...
template<> template<> void floatersObject::test<12>() {
    ...
    radia::ui::Surface surface(styleSheet);
    surface.setViewport(200.f, 160.f);
    auto lower = std::make_unique<radia::ui::Floater>();
    radia::ui::Floater* lowerTarget = lower.get();
    lower->setCanResize(true).setRect({20.f, 20.f, 100.f, 80.f});
    surface.mountFloater(std::move(lower));
    auto upper = std::make_unique<radia::ui::Floater>();
    radia::ui::Floater* upperTarget = upper.get();
    upper->setCanResize(false).setRect({70.f, 20.f, 100.f, 80.f});
    surface.mountFloater(std::move(upper));
    auto upperChild = std::make_unique<radia::ui::Button>();
    upperChild->setRect({0.f, 0.f, 100.f, 80.f}).setPointerEvents(true);
    upperTarget->content()->addChild(std::move(upperChild));

    const radia::ui::Vec2 lowerEdgeUnderUpper{lowerTarget->rect().right() - 1.f, lowerTarget->rect().y + 40.f};
    ensure("interactive child in pass-through upper Floater blocks lower resize hit testing",
           surface.pointerDown({lowerEdgeUnderUpper, radia::ui::PointerButton::Left}));
    ensure("blocked lower resize does not capture the pointer", !surface.hasPointerCapture());
    surface.pointerUp({lowerEdgeUnderUpper, radia::ui::PointerButton::Left});

    upperTarget->addClass("pass-through");
    ensure("pointer-transparent upper Floater allows lower resize", surface.pointerDown({lowerEdgeUnderUpper, radia::ui::PointerButton::Left}));
...
template<> template<> void floatersObject::test<13>() {
    ...
    ensure("native resize rejects a floater detached by its delegate", !surface.pointerDown({{99.f, 40.f}, radia::ui::PointerButton::Left}));
...
template<> template<> void floatersObject::test<14>() {
    ...
    auto overflowChild = std::make_unique<radia::ui::Button>();
    overflowChild->setRect({110.f, 20.f, 40.f, 80.f}).setPointerEvents(true);
    upperTarget->content()->addChild(std::move(overflowChild));

    const radia::ui::Vec2 point{lowerTarget->rect().right() - 1.f, lowerTarget->rect().y + 40.f};
    ensure("overflow-visible descendant remains the pointer target", surface.pointerDown({point, radia::ui::PointerButton::Left}));
...
template<> template<> void floatersObject::test<15>() {
    ...
    auto original = std::make_unique<radia::ui::Floater>();
    radia::ui::Floater* originalPointer = original.get();
    ...
    radia::ui::Floater* replacementPointer = replacement.get();
    replacement->setRect({30.f, 35.f, 120.f, 90.f});
    std::unique_ptr<radia::ui::Floater> retired = surface.replaceFloater(*originalPointer, std::move(replacement));
