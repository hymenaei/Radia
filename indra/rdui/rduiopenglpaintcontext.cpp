...
namespace rdui
{
    namespace
    {
        ...
        Rect expandedRect(const Rect& rect, float amount)
        {
            return {rect.x - amount, rect.y - amount,
                    rect.w + amount * 2.f, rect.h + amount * 2.f};
        }
        ...
        {
            return style.text_align == TextAlign::Center ? LLFontGL::HCENTER
                 : style.text_align == TextAlign::Right ? LLFontGL::RIGHT
                 : LLFontGL::LEFT;
        }
        ...
        float textX(const Rect& rect, LLFontGL::HAlign align)
        {
            return align == LLFontGL::HCENTER ? rect.x + rect.w * 0.5f
                 : align == LLFontGL::RIGHT ? rect.right()
                 : rect.x;
        }

        float textY(const Rect& rect, LLFontGL::VAlign align)
        {
            return align == LLFontGL::VCENTER ? rect.y + rect.h * 0.5f
                 : align == LLFontGL::BOTTOM ? rect.y
                 : rect.top();
        }
        ...
        void drawTexturedQuad(const Rect& rect, float u0 = 0.f, float v0 = 0.f, float u1 = 1.f, float v1 = 1.f)
        {
            ...
            gGL.texCoord2f(u0, v0); gGL.vertex2f(rect.left(), rect.bottom());
            gGL.texCoord2f(u1, v0); gGL.vertex2f(rect.right(), rect.bottom());
            gGL.texCoord2f(u1, v1); gGL.vertex2f(rect.right(), rect.top());
            gGL.texCoord2f(u0, v0); gGL.vertex2f(rect.left(), rect.bottom());
            gGL.texCoord2f(u1, v1); gGL.vertex2f(rect.right(), rect.top());
            gGL.texCoord2f(u0, v1); gGL.vertex2f(rect.left(), rect.top());
            gGL.end();
        }
    ...
    LLRenderTarget* OpenGLPaintContext::applyBlur(LLRenderTarget& source,
                                                  LLRenderTarget& horizontal_target,
                                                  LLRenderTarget& vertical_target,
                                                  const Rect& capture,
                                                  const Rect& effect_rect,
                                                  const Effect& effect,
                                                  float scale)
    {
        ...
        auto pass = [&](LLRenderTarget& input, LLRenderTarget& output, float axis_x, float axis_y)
        {
            ...
            mShapeProgram.uniform2f(blur_radii,
                                    std::min(effect.start_radius * scale, maximum_radius),
    ...
    void OpenGLPaintContext::beginEffects(const Rect& rect, const Style& style, float scale)
    {
        ...
        for (const Effect& effect : style.effects)
        {
            ...
            LLRenderTarget* blurred = applyBlur(mImpl->backgroundTargets[0], mImpl->backgroundTargets[1],
                                                mImpl->backgroundTargets[2], capture, rect, effect, frame.scale);
    ...
    void OpenGLPaintContext::drawShadow(const Rect& rect, float radius, const BoxShadow& shadow)
    {
        ...
        {
            mode = 4;
            shape = {box.x + shadow.horizontal - shadow.spread,
                     box.y - shadow.vertical - shadow.spread,
                     std::max(0.f, box.w + shadow.spread * 2.f),
    ...
    void OpenGLPaintContext::drawShapeQuad(const Rect& rect)
    {
        gGL.begin(LLRender::TRIANGLES);
        gGL.color4f(1.f, 1.f, 1.f, 1.f);
        gGL.texCoord2f(0.f, 0.f);       gGL.vertex2f(rect.left(), rect.bottom());
        gGL.texCoord2f(rect.w, 0.f);    gGL.vertex2f(rect.right(), rect.bottom());
        gGL.texCoord2f(rect.w, rect.h); gGL.vertex2f(rect.right(), rect.top());
        gGL.texCoord2f(0.f, 0.f);       gGL.vertex2f(rect.left(), rect.bottom());
        gGL.texCoord2f(rect.w, rect.h); gGL.vertex2f(rect.right(), rect.top());
        gGL.texCoord2f(0.f, rect.h);    gGL.vertex2f(rect.left(), rect.top());
        gGL.end();
    }
