...
namespace tut
{
    ...
    template<> template<>
    void rduisvg_object::test<1>()
    {
        const rdui::SvgCompileResult compiled = rdui::compileSvgIcon(
    ...
    template<> template<>
    void rduisvg_object::test<2>()
    {
        const rdui::SvgCompileResult compiled = rdui::compileSvgIcon(
            "<svg viewBox=\"0 0 24 24\"><path d=\"m21 21-4.34-4.34\"/>"
    ...
    template<> template<>
    void rduisvg_object::test<3>()
    {
        const rdui::SvgCompileResult compiled = rdui::compileSvgIcon(
            "<svg viewBox=\"0 0 24 24\"><path d=\"M20 10h-6V4\"/></svg>");
    ...
    template<> template<>
    void rduisvg_object::test<5>()
    {
        const rdui::SvgCompileResult unsupported = rdui::compileSvgIcon(
            "<svg viewBox=\"0 0 24 24\"><rect x=\"0\" y=\"0\" width=\"4\" height=\"4\"/></svg>");
        ensure("unsupported element rejected", !unsupported.ok());
        ensure("unsupported element exposes no icon", !unsupported.icon.has_value());

        const rdui::SvgCompileResult unknown_attribute = rdui::compileSvgIcon(
            "<svg viewBox=\"0 0 24 24\"><path d=\"M0 0 L4 4\" opacity=\"0.5\"/></svg>");
        ensure("unsupported attribute rejected", !unknown_attribute.ok());
    }

    template<> template<>
    void rduisvg_object::test<6>()
    {
        ensure("missing viewBox rejected",
               !rdui::compileSvgIcon("<svg><path d=\"M0 0 L4 4\"/></svg>").ok());
        ensure("invalid circle rejected",
               !rdui::compileSvgIcon("<svg viewBox=\"0 0 24 24\"><circle cx=\"1\" cy=\"2\" r=\"0\"/></svg>").ok());
        ensure("malformed XML rejected",
               !rdui::compileSvgIcon("<svg viewBox=\"0 0 24 24\"><path></svg>").ok());
        ensure("text content rejected",
               !rdui::compileSvgIcon("<svg viewBox=\"0 0 24 24\">text<path d=\"M0 0 L4 4\"/></svg>").ok());
    }
}
