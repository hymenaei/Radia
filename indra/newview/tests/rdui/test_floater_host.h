...
struct TestFloaterHost final : ComponentManager::Host {
    ...
    std::unique_ptr<rdui::Floater> unmount(rdui::Floater& root) override {
        ...
        std::unique_ptr<rdui::Floater> result = std::move(found->second);
        mounted.erase(found);
        return result;
    }
    ...
    std::map<rdui::Floater*, std::unique_ptr<rdui::Floater>> mounted;
    int replacements = 0;
    int presentations = 0;
...
private:
    bool commitReplacement(std::vector<ReplacementRequest> requests) {
        ...
        for (auto& request : requests) {
            ...
            rdui::Floater* root = request.replacement.get();
            const bool closed = request.current->closed();
            std::unique_ptr<rdui::Floater> retired = std::move(found->second);
            mounted.erase(found);
    ...
    bool commitClear(std::vector<rdui::Floater*> roots) {
        for (rdui::Floater* root : roots)
            if (!root || mounted.find(root) == mounted.end()) return false;
        for (rdui::Floater* root : roots) {
    ...
    struct Applied {
        ...
        rdui::Floater* installed;
        std::unique_ptr<rdui::Floater> retired;
    };
    ...
    void rollback(std::vector<Applied>& applied) {
        for (auto current = applied.rbegin(); current != applied.rend(); ++current) {
            ...
            std::unique_ptr<rdui::Floater> retired = std::move(current->retired);
