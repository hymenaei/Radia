...
template<> template<> void componentPersistenceObject::test<1>() {
    ...
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    const std::vector<radia::viewer::ui::ComponentKey> components{{"inventory", "one"}, {"inventory", "two"}, {"settings", {}}};
    ...
    std::vector<radia::viewer::ui::ComponentKey> restored = persistence.openComponentKeys();
    const auto order = [](const radia::viewer::ui::ComponentKey& left, const radia::viewer::ui::ComponentKey& right) {
        return std::tie(left.definitionId, left.instanceKey) < std::tie(right.definitionId, right.instanceKey);
    };
    std::vector<radia::viewer::ui::ComponentKey> expected = components;
...
template<> template<> void componentPersistenceObject::test<2>() {
    ...
    const std::vector<radia::viewer::ui::ComponentKey> restored = persistence.openComponentKeys();
    ensure_equals("only valid workspace entries are restored", restored.size(), std::size_t{2});
    ensure("keyless component is restored",
           std::find(restored.begin(), restored.end(), radia::viewer::ui::ComponentKey{"settings", {}}) != restored.end());
    ensure("keyed component is restored",
           std::find(restored.begin(), restored.end(), radia::viewer::ui::ComponentKey{"profile", "alice"}) != restored.end());
}
...
template<> template<> void componentPersistenceObject::test<3>() {
    ...
    persistence.savePlacement(radia::viewer::ui::ComponentKey{"settings", {}},
                              radia::viewer::ui::AttachedFloaterPlacement{96.f, 64.f, radia::viewer::ui::FloaterLogicalSize{480.f, 320.f}, false},
                              radia::viewer::ui::ComponentOpenState::Open);
...
template<> template<> void componentPersistenceObject::test<4>() {
    ...
    const radia::viewer::ui::ComponentKey component{"profile", "alice"};
    ...
    ensure("persistence identity round trips", radia::viewer::ui::ComponentKey::fromPersistenceKey(component.persistenceKey()) == component);
    ensure("invalid persistence separators are rejected", !radia::viewer::ui::ComponentKey::fromPersistenceKey("alice@profile@extra"));
}

template<> template<> void componentPersistenceObject::test<5>() {
    set_test_name("workspace state is saved in one combined update");
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    const std::vector<radia::viewer::ui::ComponentKey> components{{"profile", "alice"}, {"demo", {}}};
...
template<> template<> void componentPersistenceObject::test<6>() {
    ...
    const auto* attached = std::get_if<radia::viewer::ui::AttachedFloaterPlacement>(&*placement);
...
template<> template<> void componentPersistenceObject::test<7>() {
    ...
    persistence.savePlacement({"profile", "alice"}, *attached, radia::viewer::ui::ComponentOpenState::Open);
...
template<> template<> void componentPersistenceObject::test<8>() {
    ...
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    const radia::viewer::ui::ComponentKey identity{"profile", "alice"};
    persistence.savePlacement(identity, radia::viewer::ui::DetachedFloaterPlacement{100, 200, radia::viewer::ui::FloaterLogicalSize{640.f, 480.f}, true},
                              radia::viewer::ui::ComponentOpenState::Open);
    ...
    const auto* detached = std::get_if<radia::viewer::ui::DetachedFloaterPlacement>(&*restored);
...
template<> template<> void componentPersistenceObject::test<10>() {
    ...
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    const radia::viewer::ui::ComponentKey identity{"demo", {}};
    persistence.savePlacement(identity, radia::viewer::ui::DetachedFloaterPlacement{760, 120, radia::viewer::ui::FloaterLogicalSize{540.f, 680.f}, true},
                              radia::viewer::ui::ComponentOpenState::Open);
...
template<> template<> void componentPersistenceObject::test<11>() {
    ...
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    persistence.savePlacement({"demo", {}}, radia::viewer::ui::AttachedFloaterPlacement{12.f, 24.f, std::nullopt, true},
                              radia::viewer::ui::ComponentOpenState::Open);
...
template<> template<> void componentPersistenceObject::test<12>() {
    ...
    radia::viewer::ui::ComponentPersistence persistence(gSavedSettings, gSavedSettings);
    const radia::viewer::ui::ComponentKey identity{"profile", "alice"};
    persistence.savePlacement(identity, radia::viewer::ui::AttachedFloaterPlacement{10.f, 20.f, std::nullopt, false},
                              radia::viewer::ui::ComponentOpenState::Open);
    ensure("open placement creates workspace marker", gSavedSettings.getLLSD("UIWorkspace").has("alice@profile"));
    persistence.savePlacement(identity, radia::viewer::ui::AttachedFloaterPlacement{30.f, 40.f, std::nullopt, false},
                              radia::viewer::ui::ComponentOpenState::Closed);
