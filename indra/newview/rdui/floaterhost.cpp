...
public:
    ...
    FloaterReplacement(Surface& attachedSurface, DetachedFloaterManager& detachedManager, std::vector<ReplacementRequest> replacements)
        ...
        for (auto& request : replacements) {
            ...
            const bool preserveSize = rdui::detail::preserveUserResizeOnReload(current->canResize(), candidate->canResize(),
...
private:
    ...
    void rollbackOrDie() {
        if (!rollback()) LL_ERRS("rdui") << "Component Floater replacement could not be rolled back." << LL_ENDL;
    }
...
FloaterHost::FloaterHost(rdui::Surface& attachedSurface, DetachedFloaterManager& detachedManager)
    : mAttachedSurface(attachedSurface), mDetachedManager(detachedManager) {}

void FloaterHost::mount(std::unique_ptr<rdui::Floater> root) {
    mAttachedSurface.mountFloater(std::move(root));
}

std::unique_ptr<rdui::Floater> FloaterHost::unmount(rdui::Floater& root) {
...
bool FloaterHost::clearAll(std::vector<rdui::Floater*> roots) {
    for (rdui::Floater* root : roots)
        if (!root || mDetachedManager.isDetached(*root) || !mAttachedSurface.ownsFloater(*root)) return false;
    for (rdui::Floater* root : roots) {
        if (!root->closed()) root->close();
        std::unique_ptr<rdui::Floater> retired = mAttachedSurface.unmountFloater(*root);
        if (!retired || retired.get() != root) LL_ERRS("rdui") << "Component host lost a Floater during account teardown." << LL_ENDL;
    }
    return true;
}

void FloaterHost::present(rdui::Floater& root) {
    root.open();
