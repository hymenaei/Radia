...
namespace tut {
template<typename T> class MemoryValueBinding final : public radia::ui::ValueBinding<T>, public std::enable_shared_from_this<MemoryValueBinding<T>> {
public:
    explicit MemoryValueBinding(radia::ui::ValueState<T> state) : mState(std::move(state)) {}
    ...
    radia::ui::ValueBindingSubscription observe(typename radia::ui::ValueBinding<T>::Observer observer) override {
        ...
        return radia::ui::ValueBindingSubscription([weak, id] {
    ...
    void publish(radia::ui::ValueState<T> state) {
        ...
        std::vector<typename radia::ui::ValueBinding<T>::Observer> observers;
...
template<> template<> void switchObject::test<1>() {
    radia::ui::ValueState<int> clean{4, 4, std::nullopt};
    ensure("equal value and baseline are clean", !clean.dirty());
    ensure("omitted validation is valid", clean.validationStatus() == radia::ui::ValueValidationStatus::Valid);
    ensure("omitted validation has no message", clean.validationMessage() == nullptr);

    radia::ui::ValueState<int> invalid{5, 4, radia::ui::ValueValidation::invalid(radia::ui::TextSource::text("Not allowed"))};
    ensure("changed value is dirty", invalid.dirty());
    ensure("invalid status is retained", invalid.validationStatus() == radia::ui::ValueValidationStatus::Invalid);
    ...
    invalid.validation = radia::ui::ValueValidation::pending();
    ensure("pending validation is distinct", invalid.validationStatus() == radia::ui::ValueValidationStatus::Pending);
}

template<> template<> void switchObject::test<2>() {
    auto binding = std::make_shared<MemoryValueBinding<bool>>(radia::ui::ValueState<bool>{false, false, std::nullopt});
    int publications = 0;
    bool observed = false;
    radia::ui::ValueBindingSubscription subscription = binding->observe([&](const radia::ui::ValueState<bool>& state) {
        ++publications;
    ...
    binding->publish({true, true, radia::ui::ValueValidation::valid()});
