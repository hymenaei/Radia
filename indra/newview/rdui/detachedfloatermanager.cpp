...
struct DetachedFloaterManager::Impl {
    ...
    void restoreAttached(std::unique_ptr<Floater> floater, const Rect& rect) {
        if (!floater) LL_ERRS("rdui") << "Detached Floater presentation factory failed without returning the transferred Floater." << LL_ENDL;
    ...
    void detachPendingFloater() {
        ...
        if (!creation) {
            ...
            if (!returnedFloater)
                LL_ERRS("rdui") << "Detached Floater presentation factory consumed a Floater without returning it on failure." << LL_ENDL;
