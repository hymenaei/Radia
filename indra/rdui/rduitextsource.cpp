...
namespace rdui
{
    ...
    TextSourceNode TextSourceNode::localized(
        LocalizationRequest request, InlineContent fallback)
    {
        return TextSourceNode(Localized{
            std::move(request), std::move(fallback)});
    }

    TextSourceNode TextSourceNode::container(
        InlineContentKind kind, std::vector<TextSourceNode> children)
    {
        llassert_always(kind == InlineContentKind::B
                        || kind == InlineContentKind::I
                        || kind == InlineContentKind::S);
        ...
        TextSourceNode sourceNode(const InlineContentNode& node)
        {
            ...
            switch (node.kind())
            {
                case InlineContentKind::Text:
                    return TextSourceNode::text(node.value());
                case InlineContentKind::B:
                case InlineContentKind::I:
                case InlineContentKind::S:
                    return TextSourceNode::container(node.kind(), std::move(children));
                case InlineContentKind::Kbd:
                    return TextSourceNode::kbd(node.metadata(), node.keybindingPresentation());
                case InlineContentKind::Br:
                    return TextSourceNode::br();
                case InlineContentKind::Link:
                    return TextSourceNode::link(node.metadata(), std::move(children));
            }
            llassert(false);
            return TextSourceNode::br();
        }

        std::vector<InlineContentNode> materializeNodes(
            const std::vector<TextSourceNode>& source,
