...
public:
    ...
    // Pins one shared-arena generation for a complete analytic text batch.
    // Starting the next batch performs any deferred eviction only after the
    // renderer has drained vertices that reference the previous generation.
    // Requiring this token on lookup makes stale-offset use unrepresentable in
    // normal code instead of relying on a caller-side flush/reset protocol.
    class Batch
    {
    public:
    ...
    // Begin one generation-pinned render batch. If a previous batch overflowed
    // the shared arena, this drains its queued vertices and performs the reset
    // before returning the new token.
...
#ifdef LL_TEST
    // Test-only clean-slate hook. Production eviction remains owned entirely by
    // beginBatch(), so callers cannot bypass the pinned-generation protocol.
...
private:
    // Clear the shared arena + bump the generation. Only beginBatch() may evict
    // in production, after it has established a safe render boundary.
    static void reset();
    ...
    // One million texels = 8 MB shared across every face and size. Eviction is
    // a whole-store generation reset, so a little more resident memory avoids
    // visible re-encode/upload stalls in multilingual, long-running sessions.
    ...
    static bool sResetPending;       // overflow observed; reset at a safe boundary
    static U32 sActiveBatches;       // enforces the single-threaded pin contract
