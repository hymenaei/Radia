...
using RuntimeWidget = rdui::Widget;
...
struct ComponentController::Impl {
    ...
    std::vector<rdui::EventRegistrationDescriptor> registrations;
...
void warnUnsupported(const std::shared_ptr<detail::ControllerWidgetSlot>& slot, const char* operation, const RuntimeWidget& widget) {
    ...
    LL_WARNS("rdui") << "Controller Widget " << slot->id << " does not support " << operation << " on <" << widget.elementName() << ">." << LL_ENDL;
}
...
ComponentController::PreparedWidgets ComponentController::prepareWidgets(RuntimeWidget& root, DiagnosticResult& result) {
    ...
    prepared.mRoot = &root;
    rdui::detail::indexWidgetsInScope(root, prepared.index);
...
ComponentController::PreparedMountResult ComponentController::prepare(RuntimeWidget& root, SettingResolver& settingResolver) {
    ...
    for (const rdui::EventRegistrationDescriptor& registration : mImpl->registrations)
        binder.event(rdui::detail::makeEventRegistration(registration));
...
void ComponentController::commit(PreparedMount&& prepared) {
    if (!prepared.mState || prepared.mState->controller != this || !prepared.mState->root)
        LL_ERRS("rdui") << "ComponentController committed an invalid prepared mount." << LL_ENDL;
...
void ComponentController::addEventRegistration(rdui::EventRegistrationDescriptor registration) {
    if (mImpl->registrationsSealed) {
        LL_WARNS("rdui") << "Controller Event Handler registration was attempted after preparation began." << LL_ENDL;
        return;
    }
