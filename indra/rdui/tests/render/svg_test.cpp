...
template<> template<> void svgObject::test<1>() {
    ...
    const rdui::SvgCompileResult compiled = rdui::compileSvgIcon(kSvg);
    ...
    ensure_equals("round cap", static_cast<int>(icon.strokeCap), static_cast<int>(rdui::StrokeCap::Round));
}

template<> template<> void svgObject::test<2>() {
    const rdui::SvgCompileResult compiled =
        rdui::compileSvgIcon("<svg viewBox=\"0 0 24 24\"><path d=\"m21 21-4.34-4.34\"/><circle cx=\"11\" cy=\"11\" r=\"8\"/></svg>");
    ensure("path and circle SVG compiles", compiled.ok());
    const rdui::SvgIcon& icon = *compiled.icon;
    ensure_equals("path and circle", icon.paths.size(), 2U);
    ensure_equals("default butt", static_cast<int>(icon.strokeCap), static_cast<int>(rdui::StrokeCap::Butt));
...
template<> template<> void svgObject::test<3>() {
    ...
    const rdui::SvgIcon& icon = *compiled.icon;
    const rdui::Path transformed = rdui::transformSvgPath(icon.paths[0], icon.viewBox, {10.f, 20.f, 16.f, 16.f});
...
template<> template<> void svgObject::test<5>() {
    ...
    const rdui::SvgCompileResult unknownAttribute = rdui::compileSvgIcon("<svg viewBox=\"0 0 24 24\"><path d=\"M0 0 L4 4\" opacity=\"0.5\"/></svg>");
