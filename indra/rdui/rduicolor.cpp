...
namespace rdui
{
    namespace
    {
        ...
        Color hwb(float hue, float whiteness, float blackness, float alpha)
        {
            ...
            return {
                base.r * scale + whiteness,
                base.g * scale + whiteness,
                base.b * scale + whiteness,
                alpha
            };
        }
        ...
        Color lab(float lightness, float a, float b, float alpha)
        {
            ...
            return linearSrgb(
                3.240969942f * x65 - 1.537383178f * y65 - .498610760f * z65,
                -.969243636f * x65 + 1.875967502f * y65 + .041555057f * z65,
                .055630080f * x65 - .203976959f * y65 + 1.056971514f * z65,
                alpha);
        }
        ...
        Color oklab(float lightness, float a, float b, float alpha)
        {
            ...
            return linearSrgb(
                4.0767416621f * l - 3.3077115913f * m + .2309699292f * s,
        ...
        std::optional<Color> parseHex(const std::string& value)
        {
            ...
            if (value.size() <= 5)
                return Color(shortChannel(1), shortChannel(2), shortChannel(3), value.size() == 5 ? shortChannel(4) : 1.f);
    ...
    bool isColorSyntax(const std::string& raw)
    {
        ...
        return (!value.empty() && value.front() == '#') || value == "transparent"
            || value.rfind("rgb(", 0) == 0 || value.rfind("hsl(", 0) == 0
            || value.rfind("hwb(", 0) == 0 || value.rfind("lab(", 0) == 0
            || value.rfind("lch(", 0) == 0 || value.rfind("oklab(", 0) == 0
    ...
    std::optional<Color> parseColor(const std::string& raw)
    {
        ...
        if (name != "rgb" && name != "hsl" && name != "hwb" && name != "lab"
            && name != "lch" && name != "oklab" && name != "oklch") return std::nullopt;
        ...
        {
            Color result;
            if (!parseRgbChannel(channels[0], result.r) || !parseRgbChannel(channels[1], result.g) || !parseRgbChannel(channels[2], result.b)) return std::nullopt;
            result.a = alpha;
            return result;
        }
