...
public:
    static Vec2 measure(Widget& node, const StyleSheet& theme, const TextMetrics& text_metrics, std::optional<float> outer_width = std::nullopt) {
        ...
        if (!outer_width && !node.mMeasureDirty) return node.mDesiredSize;

        const Style style = resolveWidgetStyle(theme, node);
        const Flow flow = style.flow;
        std::optional<float> resolved_width = outer_width;
        if (!resolved_width && !style.width.isAuto() && !style.width.isPercentage())
            resolved_width = styledDimension(style.width, style.min_width, 0.f);
        Style intrinsic_style = style;
        if (resolved_width) intrinsic_style.width = Dimension::fromPixels(std::max(0.f, *resolved_width));
        const Vec2 intrinsic = node.intrinsicSize(theme, intrinsic_style, text_metrics);
        ...
        for (const auto& child_ptr : node.mChildren) {
            ...
            if (flow == Flow::Column && resolved_width) {
                const float available_cross = std::max(0.f, *resolved_width - style.padding.horizontal());
                child_size.x = styledDimension(child_style.width, child_style.min_width, child_size.x, available_cross);
                applyCrossAxisSizing(child_size, child_style, flow, available_cross, crossAlignment(style, child_style, flow));
                if (child_style.height.isAuto() && !child_style.aspect_ratio) child_size.y = measure(child, theme, text_metrics, child_size.x).y;
            ...
            if ((flow == Flow::Row || flow == Flow::Column) && !child_style.flex_basis.isAuto()) {
                ...
                if (!percentage_is_auto) {
                    const float parent_size = parent_dimension.isAuto() ? rect_size : parent_dimension.resolve(0.f, rect_size);
            ...
            if (flow == Flow::Row) {
                row_layouts.push_back({&child, child_style, child_size, child_automatic_minimum});
                if (child.flowBreakBefore() && row_children) finishRow();
                if (previous_child && node.hasLayoutGapBetween(*previous_child, child)) row_width += fixed_gap;
                if (previous_child) row_width -= node.layoutOverlapBetween(*previous_child, child, style);
                row_width += child_outer_width;
                row_height = std::max(row_height, outer_height);
                ++row_children;
            } else if (flow == Flow::Column) {
                if (previous_child && node.hasLayoutGapBetween(*previous_child, child)) content.y += fixed_gap;
                if (previous_child) content.y -= node.layoutOverlapBetween(*previous_child, child, style);
        ...
        if (flow == Flow::Row) {
            ...
            if (resolved_width && !row_layouts.empty()) {
                ...
                for (ChildLayout& child : row_layouts) {
                ...
                const auto lines = rowLines(row_layouts);
                for (const auto& [begin, end] : lines) {
                    allocateRowLine(node, row_layouts, begin, end, style, available_main);
                    remeasureRowChildren(row_layouts, begin, end, theme, text_metrics);
                }
        ...
        const Vec2 desired = {resolved_width ? styledDimension(Dimension::fromPixels(*resolved_width), style.min_width, natural.x)
...
private:
    ...
    static std::vector<std::pair<std::size_t, std::size_t>> rowLines(const std::vector<ChildLayout>& children) {
    ...
    static RowAllocation allocateRowLine(Widget& parent, std::vector<ChildLayout>& children, std::size_t begin, std::size_t end,
                                         const Style& parent_style, float available_main) {
        float total = 0.f;
        int auto_margins = 0;
        for (std::size_t index = begin; index < end; ++index) {
            const ChildLayout& child = children[index];
        ...
        for (std::size_t index = begin + 1; index < end; ++index) {
            if (parent.hasLayoutGapBetween(*children[index - 1].node, *children[index].node)) ++gap_count;
            overlap += parent.layoutOverlapBetween(*children[index - 1].node, *children[index].node, parent_style);
        }
        ...
        distributeFlexSpace(children, begin, end, Flow::Row, available_main, !auto_margins && !parent_style.gap.isAuto(), total);
    ...
    static void remeasureRowChildren(std::vector<ChildLayout>& children, std::size_t begin, std::size_t end, const StyleSheet& theme,
                                     const TextMetrics& text_metrics) {
        for (std::size_t index = begin; index < end; ++index) {
            ChildLayout& child = children[index];
            if (!child.style.height.isAuto() || child.style.aspect_ratio) continue;
            child.measured.y = measure(*child.node, theme, text_metrics, child.measured.x).y;
            child.automatic_minimum.y = child.measured.y;
        }
    }
    ...
    static std::vector<ChildLayout> flowChildren(Widget& parent, const StyleSheet& theme, const TextMetrics& text_metrics, Flow flow) {
        ...
        if (flow == Flow::Row || flow == Flow::Column) {
            std::stable_sort(result.begin(), result.end(),
                             [](const ChildLayout& left, const ChildLayout& right) { return left.style.order < right.style.order; });
        }
        return result;
    }

    static Rect positionedRect(const ChildLayout& child, const Rect& parent, VerticalAlign vertical_alignment) {
        const bool explicit_rect = child.node->mRectExplicit;
        const float width = explicit_rect && child.style.width.isAuto()
            ? child.style.min_width ? std::max(child.node->mRect.w, child.style.min_width->resolve(parent.w)) : child.node->mRect.w
        ...
        const float height = explicit_rect && child.style.height.isAuto()
            ? child.style.min_height ? std::max(child.node->mRect.h, child.style.min_height->resolve(parent.h)) : child.node->mRect.h
    ...
    static void arrangeNode(Widget& node, const StyleSheet& theme, const TextMetrics& text_metrics, LayoutDirection direction) {
        ...
        if (flow == Flow::Row) {
            ...
            const auto lines = rowLines(children);
            for (const auto& [begin, end] : lines) {
                ...
                for (std::size_t index = begin; index < end; ++index) {
                    applyCrossAxisSizing(children[index].measured, children[index].style, flow, preliminary_height,
                                         crossAlignment(parent_style, children[index].style, flow));
                }
            }

            std::vector<RowAllocation> allocations;
            allocations.reserve(lines.size());
            for (const auto& [begin, end] : lines) {
                allocations.push_back(allocateRowLine(node, children, begin, end, parent_style, available_main));
                remeasureRowChildren(children, begin, end, theme, text_metrics);
            }
            ...
            for (const auto& [begin, end] : lines) {
                ...
                line_heights.push_back(lines.size() == 1 && available_cross >= 0.f ? available_cross : height);
            }
            for (std::size_t line = 0; line < lines.size(); ++line) {
                const auto [begin, end] = lines[line];
                for (std::size_t index = begin; index < end; ++index)
                    applyCrossAxisSizing(children[index].measured, children[index].style, flow, line_heights[line],
                                         crossAlignment(parent_style, children[index].style, flow));
            }
            ...
            for (std::size_t line = 0; line < lines.size(); ++line) {
                ...
                const RowAllocation& allocation = allocations[line];
                ...
                for (std::size_t index = begin; index < end; ++index) {
                    ...
                    if (rtl) {
                        ...
                        if (index + 1 < end) {
                            if (node.hasLayoutGapBetween(*child.node, *children[index + 1].node)) x -= gap;
                            x += node.layoutOverlapBetween(*child.node, *children[index + 1].node, parent_style);
                        }
                    } else {
            ...
            for (ChildLayout& child : children) {
                ...
                applyCrossAxisSizing(child.measured, child.style, flow, available_cross, crossAlignment(parent_style, child.style, flow));
            ...
            for (std::size_t index = 1; index < children.size(); ++index)
                if (node.hasLayoutGapBetween(*children[index - 1].node, *children[index].node)) ++gap_count;
            float overlap = 0.f;
            for (std::size_t index = 1; index < children.size(); ++index)
                overlap += node.layoutOverlapBetween(*children[index - 1].node, *children[index].node, parent_style);
            float gap = parent_style.gap.fixedPixels();
            total += gap * static_cast<float>(gap_count) - overlap;
            distributeFlexSpace(children, 0, children.size(), flow, available_main, !auto_margins && !parent_style.gap.isAuto(), total);
            ...
            for (std::size_t i = 0; i < children.size(); ++i) {
                ...
                if (i) {
                    ...
                    y += node.layoutOverlapBetween(*children[i - 1].node, *child.node, parent_style);
                }
