...
{
    template<typename T>
    class MemoryValueBinding final : public rdui::ValueBinding<T>,
                                     public std::enable_shared_from_this<MemoryValueBinding<T>>
    {
        public:
    ...
    template<> template<>
    void rduivaluebinding_object::test<1>()
    {
        rdui::ValueState<int> clean{4, 4, std::nullopt};
        ensure("equal value and baseline are clean", !clean.dirty());
        ensure("omitted validation is valid",
               clean.validationStatus() == rdui::ValueValidationStatus::Valid);
        ...
        invalid.validation = rdui::ValueValidation::pending();
               ...
               invalid.validationStatus() == rdui::ValueValidationStatus::Pending);
    }

    template<> template<>
    void rduivaluebinding_object::test<2>()
    {
        auto binding = std::make_shared<MemoryValueBinding<bool>>(
            rdui::ValueState<bool>{false, false, std::nullopt});
        int publications = 0;
        bool observed = false;
        rdui::ValueBindingSubscription subscription = binding->observe(
            [&](const rdui::ValueState<bool>& state)
            {
                ++publications;
