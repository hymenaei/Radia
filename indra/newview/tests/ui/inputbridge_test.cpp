...
template<> template<> void inputBridgeObject::test<1>() {
    ...
    const radia::viewer::ui::NativePointerInput input{
        12.5f, 34.25f, radia::viewer::ui::NativePointerButton::Auxiliary1, MASK_SHIFT | MASK_ALT, 2, 3.5f, -4.f,
    };

    const radia::ui::PointerEvent translated = radia::viewer::ui::translatePointerInput(input);
...
template<> template<> void inputBridgeObject::test<2>() {
    ...
    const radia::viewer::ui::NativeKeyInput input{
        KEY_PAD_RETURN,
...
template<> template<> void inputBridgeObject::test<4>() {
    ...
    ensure("pointer cursor", radia::viewer::ui::translateCursor(radia::ui::CursorStyle::Pointer) == UI_CURSOR_HAND);
    ensure("horizontal resize cursor", radia::viewer::ui::translateCursor(radia::ui::CursorStyle::EastWestResize) == UI_CURSOR_SIZEWE);
    ensure("fallback cursor", radia::viewer::ui::translateCursor(radia::ui::CursorStyle::Auto) == UI_CURSOR_ARROW);
}
} // namespace tut
