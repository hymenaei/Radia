/**
 * @file rduifloaterdocumentmanager_test.cpp
 * @brief
 *
...
struct floater_document_manager {
    ...
    public:
        Controller(std::string resource, ControllerState& state) : mResource(std::move(resource)), mState(state) {}

        std::string resourceId() const override { return mResource; }
        rdui::PreparedBindingResult prepareBindings(rdui::Floater& floater) override { return rdui::Binder(floater).prepare(); }
        void commitBindings(rdui::PreparedBinding&& prepared) override { mBinding = prepared.commit(); }
...
locales:
  en:
    ...
    bool registerOne(const std::string& definition = "one") {
        return manager.registerDefinition(definition, [this](rdui::System&) { return std::make_unique<Controller>("one.xml", controllerState); });
    }
...
template<> template<> void floater_document_manager_object::test<1>() {
...
template<> template<> void floater_document_manager_object::test<2>() {
...
template<> template<> void floater_document_manager_object::test<3>() {
...
template<> template<> void floater_document_manager_object::test<4>() {
...
template<> template<> void floater_document_manager_object::test<5>() {
