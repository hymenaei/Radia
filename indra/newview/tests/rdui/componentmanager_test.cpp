...
class TestBooleanBinding final : public rdui::ValueBinding<bool> {
public:
    explicit TestBooleanBinding(bool value) : mState{value, value, std::nullopt} {}

    rdui::ValueState<bool> state() const override { return mState; }
    void write(bool value) override {
        mState.value = value;
        notify();
    }
    rdui::ValueBindingSubscription observe(Observer observer) override {
...
public:
    ...
    rdui::SettingResolution resolve(std::string_view settingName, std::type_index requestedType) override {
        if (settingName != "test-enabled") return {rdui::SettingResolution::ResolutionStatus::Missing, {}};
        if (requestedType != typeid(bool)) return {rdui::SettingResolution::ResolutionStatus::TypeMismatch, {}};
...
struct componentManagerData {
    ...
    class Controller final : public rdui::viewer::ComponentController {
    public:
        ...
        void onReloadFailed(const rdui::DiagnosticResult&) override { ++mState.failed; }
    ...
    private:
        ...
        void inspect(int index, std::string destination, bool enabled, rdui::viewer::Widget source, const rdui::viewer::ClickEvent& event) {
            mState.multipleArguments =
                index == 4 && destination == "settings" && enabled && source.id() == "inspect" && event.source().id() == source.id();
        }

        void changed(const rdui::viewer::ChangeEvent& event) { mState.changeObserved = event.checked; }
        void doubleClick(const rdui::viewer::MouseWidgetEvent&) { mState.doubleClickObserved = true; }
        void mouseDown(const rdui::viewer::MouseWidgetEvent&) { mState.mouseDownObserved = true; }
        void mouseUp(const rdui::viewer::MouseWidgetEvent&) { mState.mouseUpObserved = true; }
        void mouseMove(const rdui::viewer::MouseWidgetEvent&) { mState.mouseMoveObserved = true; }
        void longClick(const rdui::viewer::LongClickEvent&) { mState.longClickObserved = true; }
        void contextMenu(const rdui::viewer::MouseWidgetEvent&) { mState.contextMenuObserved = true; }

        ControllerState& mState;
        rdui::viewer::Widget& mStatus = getWidgetById("status");
    };

    class ForeignController final : public rdui::viewer::ComponentController {
    public:
        explicit ForeignController(rdui::System& system) : rdui::viewer::ComponentController(system) {}
        void wrongTarget() {}
    };

    class MismatchedController final : public rdui::viewer::ComponentController {
    public:
        explicit MismatchedController(rdui::System& system) : rdui::viewer::ComponentController(system) {
    ...
    static rdui::SkinGenerationPrepareResult prepareGeneration(bool includeStatus = true) {
    ...
    bool registerOne(const std::string& definition = "one") {
        return manager.registerDefinition(definition, "one.xml",
                                          [this](rdui::System& system) { return std::make_unique<Controller>(system, controllerState); });
    }

    bool registerEmpty() {
        return manager.registerDefinition("empty", "empty.xml", [this](rdui::System& system) {
            class EmptyController final : public rdui::viewer::ComponentController {
            public:
                explicit EmptyController(rdui::System& system) : rdui::viewer::ComponentController(system) {}
            };
    ...
    bool registerMismatched() {
        return manager.registerDefinition("mismatched", "empty.xml",
                                          [this](rdui::System& system) { return std::make_unique<MismatchedController>(system); });
    }

    std::vector<rdui::Floater*> liveFloaters() const {
        std::vector<rdui::Floater*> result;
        for (const auto& [root, floater] : host.mounted)
            if (floater && !floater->closed()) result.push_back(floater.get());
        return result;
    }

    std::optional<rdui::viewer::ComponentKey> keyFor(const rdui::Floater& floater) const { return manager.componentKeyFor(floater); }

    rdui::Floater* mountedFor(const rdui::viewer::ComponentKey& key) const {
        for (const auto& [root, floater] : host.mounted)
    ...
    rdui::viewer::ComponentManager manager;
};
...
template<> template<> void componentManagerObject::test<2>() {
    ...
    ensure("first stable identity mounted", manager.componentKeyFor(*first.floater) == rdui::viewer::ComponentKey{"profile", "alice"});
    ensure("second stable identity mounted", manager.componentKeyFor(*second.floater) == rdui::viewer::ComponentKey{"profile", "bob"});
...
template<> template<> void componentManagerObject::test<4>() {
    ...
    auto* press = dynamic_cast<rdui::Button*>(findWidget(*opened.floater, "press"));
    auto* inspect = dynamic_cast<rdui::Button*>(findWidget(*opened.floater, "inspect"));
    auto* status = dynamic_cast<rdui::Text*>(findWidget(*opened.floater, "status"));
    ensure("zero-argument Event source exists", press != nullptr);
    ensure("multi-argument Event source exists", inspect != nullptr);
    auto* events = dynamic_cast<rdui::Button*>(findWidget(*opened.floater, "events"));
    auto* changed = dynamic_cast<rdui::Switch*>(findWidget(*opened.floater, "changed"));
    ensure("typed Event source exists", events != nullptr);
    ensure("typed Change source exists", changed != nullptr);
    ensure("double-click Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::DoubleClick));
    ensure("mouse-down Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::MouseDown));
    ensure("mouse-up Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::MouseUp));
    ensure("mouse-move Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::MouseMove));
    ensure("long-click Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::LongClick));
    ensure("context-menu Event Call is bound", events && events->eventCall(rdui::WidgetEventKind::ContextMenu));
    press->activate();
    inspect->activate();
    changed->activate();
    ...
    rdui::Floater* replacement = liveFloaters().front();
    manager.idle();
...
template<> template<> void componentManagerObject::test<5>() {
    ...
    auto openComponents = [&] {
        std::vector<rdui::viewer::ComponentKey> result;
        manager.forEachOpen([&](const rdui::viewer::ComponentKey& key, rdui::Floater&) { result.push_back(key); });
        return result;
    };

    second.floater->close();
    const std::vector<rdui::viewer::ComponentKey> closed = openComponents();
...
template<> template<> void componentManagerObject::test<6>() {
    ...
    rdui::SkinGenerationPrepareResult generation = prepareGeneration(false);
    ensure("candidate generation prepared", generation.ok());
    auto prepared = manager.prepareReplacement(*generation.generation, "en");
    ...
    auto* press = dynamic_cast<rdui::Button*>(findWidget(*liveFloaters().front(), "press"));
...
template<> template<> void componentManagerObject::test<8>() {
    ...
    rdui::WidgetRef<rdui::Floater> evictedRoot(opened.floater);
    opened.floater->close();
...
template<> template<> void componentManagerObject::test<9>() {
    ...
    Host temporaryHost;
    std::optional<rdui::viewer::ComponentManager::PreparedReplacement> orphan;
    {
        rdui::viewer::ComponentManager temporary(system, temporaryHost, resolver);
        ensure("definition registered", temporary.registerDefinition("one", "one.xml", [this](rdui::System& system) {
...
template<> template<> void componentManagerObject::test<11>() {
    ...
    rdui::Floater* originalFirst = first.floater;
    rdui::Floater* originalSecond = second.floater;
...
template<> template<> void componentManagerObject::test<12>() {
    ...
    ensure("empty root mounted", opened.floater != nullptr && manager.componentKeyFor(*opened.floater) == rdui::viewer::ComponentKey{"empty", {}});
}
...
template<> template<> void componentManagerObject::test<15>() {
    ...
    auto* setting = dynamic_cast<rdui::Switch*>(findWidget(*opened.floater, "setting"));
...
template<> template<> void componentManagerObject::test<19>() {
    ...
    ensure("first root remains mapped", manager.componentKeyFor(*first.floater) == rdui::viewer::ComponentKey{"one", {}});
    ensure("second root remains mapped", manager.componentKeyFor(*second.floater) == rdui::viewer::ComponentKey{"two", {}});
