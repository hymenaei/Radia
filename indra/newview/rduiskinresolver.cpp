...
namespace rdui::viewer
{
    namespace
    {
        ...
        void validateKeys(const LLSD& object, std::initializer_list<const char*> allowed,
                          DiagnosticResult& result, const std::string& source, const std::string& context)
        {
            if (!object.isMap()) return;
            for (auto entry = object.beginMap(); entry != object.endMap(); ++entry)
                if (!allowedKey(entry->first, allowed))
                    result.error("skin.manifest.key_unknown",
                                 "Unknown " + context + " key: " + entry->first + ".", source);
        }

        bool requireString(const LLSD& object, const char* key, std::string& value,
        ...
        std::optional<std::filesystem::path> resourcePath(
            const LLSD& radia, const char* key, const std::filesystem::path& skin_root,
            bool directory, DiagnosticResult& result, const std::string& source)
        {
            if (!radia.has(key)) return std::nullopt;
            if (!radia[key].isString() || radia[key].asString().empty())
            {
                result.error("skin.manifest.resource.invalid",
                             "Radia resource '" + std::string(key) + "' must be a non-empty relative path.", source);
                return std::nullopt;
            }
            ...
            {
                result.error("skin.manifest.resource.missing",
                             "Radia resource is missing or outside its Skin: " + value + ".", source);
                return std::nullopt;
            }
            const bool correct_type = directory
                ? std::filesystem::is_directory(resolved, error)
                : std::filesystem::is_regular_file(resolved, error);
            if (error || !correct_type)
            {
                result.error("skin.manifest.resource.type_invalid",
                             "Radia resource has the wrong filesystem type: " + value + ".", source);
                return std::nullopt;
            }
            return resolved;
        }
        ...
        ManifestResult parseManifest(const std::filesystem::path& root)
        {
            ...
            validateKeys(document, {"id", "name", "author", "url", "notes", "preview", "base", "radia"},
            ...
            if (!document.has("base"))
                result.error("skin.manifest.field_missing", "Missing required manifest field: base.", source);
            ...
            if (!document.has("radia"))
                result.error("skin.manifest.field_missing", "Missing required manifest field: radia.", source);
            else if (!document["radia"].isMap())
                result.error("skin.manifest.radia_invalid", "Manifest radia field must be an object.", source);
            else
            {
                ...
                if (root_skin)
                {
                    for (const char* key : {"stylesheet", "layouts", "localization", "assets"})
                        if (!radia.has(key)) result.error("skin.manifest.resource.required",
                            "Root Skin must declare Radia resource: " + std::string(key) + ".", source);
                }
                else if (radia.size() == 0)
            ...
            for (const char* optional : {"url", "notes", "preview"})
                if (document.has(optional) && !document[optional].isString())
                    result.error("skin.manifest.field_invalid",
                                 "Optional manifest field '" + std::string(optional) + "' must be a string.", source);
        ...
        void addLayer(const SkinManifest& manifest, const std::filesystem::path& path,
                      std::vector<ResourceLayer>& layers, SkinSnapshotResult& result)
        {
        ...
        void addStyleLayer(const SkinManifest& manifest, const std::filesystem::path& entrypoint,
                           std::vector<ResourceLayer>& layers, SkinSnapshotResult& result)
        {
            const std::optional<std::string> source = readFile(entrypoint);
            if (!source)
            {
                result.error("skin.resource.read_failed", "Could not read declared Skin resource.",
                             sourceName(manifest, entrypoint));
                return;
            }

            std::error_code error;
            const std::filesystem::path relative_entrypoint =
                std::filesystem::relative(entrypoint, manifest.root, error);
            if (error)
            {
            ...
            for (std::filesystem::recursive_directory_iterator iterator(manifest.root, error), end;
                 iterator != end && !error; iterator.increment(error))
            {
                std::error_code file_error;
                if (!iterator->is_regular_file(file_error) || file_error
                    || iterator->path().extension() != ".radia")
                {
                    continue;
                }
                const std::filesystem::path canonical_file =
                    std::filesystem::canonical(iterator->path(), file_error);
                if (file_error || !inside(canonical_file, manifest.root)) continue;
                const std::filesystem::path relative =
                    std::filesystem::relative(canonical_file, manifest.root, file_error);
            ...
            if (error)
            {
                result.error("skin.resource.discovery_failed",
                             "Could not enumerate stylesheet modules inside the Skin.",
        ...
        void overlayDirectory(const SkinManifest& manifest, const std::filesystem::path& root,
                              const std::string& logical_prefix, ResourceSnapshot& snapshot,
    ...
    SkinSnapshotResult SkinResolver::resolve(
        ...
        const std::vector<std::filesystem::path>& installed_roots) const
    {
        ...
        for (const std::filesystem::path& catalog_root : installed_roots)
        {
            ...
            if (error) continue;
            for (std::filesystem::directory_iterator iterator(catalog_root, error), end;
                 iterator != end && !error; iterator.increment(error))
            {
