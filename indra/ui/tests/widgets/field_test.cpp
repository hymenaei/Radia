...
namespace {
void bindChangeEvent(radia::ui::Binder& binder, std::string name, std::function<void(const radia::ui::ChangeEvent&)> callback) {
    binder.event(radia::ui::detail::makeEventRegistration(
        std::move(name), radia::ui::WidgetEventKind::Change,
        [callback = std::move(callback)](const radia::ui::WidgetEvent& event, const radia::ui::EventCall&) {
            callback(static_cast<const radia::ui::ChangeEvent&>(event));
        },
        [](const radia::ui::EventCall& call, radia::ui::WidgetEventKind) { return call.arguments().empty() ? nullptr : "binding.event.arity_mismatch"; }));
}
} // namespace
...
template<> template<> void fieldObject::test<1>() {
    ...
    const radia::ui::LayoutBuildResult result = factory.buildWidgetTreeFromString(kFieldLayout, "field.xml");
    auto* field = result.rootAs<radia::ui::Field>();
    ...
    const radia::ui::LayoutBuildResult linkResult = factory.buildWidgetTreeFromString(kLinkLayout, "link.xml");
    ...
    const radia::ui::LayoutBuildResult standaloneHint = factory.buildWidgetTreeFromString("<hint>orphan</hint>", "hint.xml");
    ...
    const radia::ui::LayoutBuildResult standaloneError = factory.buildWidgetTreeFromString("<error>orphan</error>", "error.xml");
    ...
    const radia::ui::LayoutBuildResult duplicateHint = factory.buildWidgetTreeFromString(kDuplicateHintLayout, "duplicate-hint.xml");
    ...
    const radia::ui::LayoutBuildResult missing = factory.buildWidgetTreeFromString("<field/>", "field-missing.xml");
    ...
    const radia::ui::LayoutBuildResult unsupported =
    ...
    const radia::ui::LayoutBuildResult mismatch = factory.buildWidgetTreeFromString(kMismatchLayout, "field-target.xml");
    ...
    const radia::ui::LayoutBuildResult leadingBreak = factory.buildWidgetTreeFromString("<panel><br/><button/></panel>", "leading-break.xml");
    ...
    const radia::ui::LayoutBuildResult trailingBreak = factory.buildWidgetTreeFromString("<panel><button/><br/></panel>", "trailing-break.xml");
    ...
    const radia::ui::LayoutBuildResult consecutiveBreak =
    ...
    const radia::ui::LayoutBuildResult attributedBreak =
    ...
    const radia::ui::LayoutBuildResult attributedHint = factory.buildWidgetTreeFromString(kAttributedHintLayout, "attributed-hint.xml");
    ...
    const radia::ui::LayoutBuildResult chromeBreak =
...
template<> template<> void fieldObject::test<2>() {
    ...
    radia::ui::LayoutBuildResult result = factory.buildWidgetTreeFromString(kInlineLayout, "inline.xml");
    auto text = result.root ? requireWidget<radia::ui::Text>(*result.root, "copy") : radia::ui::WidgetRef<radia::ui::Text>();
    auto title = result.root ? requireWidget<radia::ui::Text>(*result.root, "title") : radia::ui::WidgetRef<radia::ui::Text>();
    ...
    ensure_equals("plain content remains a text value", static_cast<int>(text->content().nodes()[0].kind()),
                  static_cast<int>(radia::ui::InlineContentKind::Text));
    ...
    ensure_equals("explicit line break remains an inline value", static_cast<int>(text->content().nodes()[3].kind()),
                  static_cast<int>(radia::ui::InlineContentKind::Br));
    const char* kLabelLayout =
        "<panel><label id=\"label\" for=\"target\">name <b>important</b><br/><i>detail</i></label><switch id=\"target\"/></panel>";
    radia::ui::LayoutBuildResult labelResult = factory.buildWidgetTreeFromString(kLabelLayout, "label-inline.xml");
    auto label = labelResult.root ? requireWidget<radia::ui::Label>(*labelResult.root, "label") : radia::ui::WidgetRef<radia::ui::Label>();
    ...
    const radia::ui::LayoutBuildResult decoration =
        factory.buildWidgetTreeFromString("<text><s>outdated</s> <kbd shortcut=\"toggle-fly\"/></text>", "decoration.xml");
    const auto* decorated = decoration.rootAs<radia::ui::Text>();
    ...
    ensure_equals("strike remains a semantic inline node", static_cast<int>(decorated->content().nodes()[0].kind()),
                  static_cast<int>(radia::ui::InlineContentKind::S));
    ensure_equals("keybinding command id remains a shortcut id", decorated->content().nodes()[2].shortcutId(), std::string("toggle-fly"));

    const radia::ui::LayoutBuildResult strikeSpacing = factory.buildWidgetTreeFromString("<text>keep <s>remove</s></text>", "strike-spacing.xml");
    const auto* struck = strikeSpacing.rootAs<radia::ui::Text>();
    ...
    const radia::ui::LayoutBuildResult missingShortcut = factory.buildWidgetTreeFromString("<text><kbd/></text>", "missing-kbd-shortcut.xml");
    ...
    const radia::ui::LayoutBuildResult invalidShortcut =
    ...
    const radia::ui::LayoutBuildResult widgetChild = factory.buildWidgetTreeFromString("<text><label>not-inline</label></text>", "widget-child.xml");
...
template<> template<> void fieldObject::test<3>() {
    ...
    radia::ui::LayoutBuildResult result = factory.buildWidgetTreeFromString(kLabelTargetLayout, "label-target.xml");
    auto label = result.root ? requireWidget<radia::ui::Label>(*result.root, "toggleLabel") : radia::ui::WidgetRef<radia::ui::Label>();
    auto target = result.root ? requireWidget<radia::ui::Switch>(*result.root, "toggle") : radia::ui::WidgetRef<radia::ui::Switch>();
    ...
    int changes = 0;
    radia::ui::Binder binder(*result.root);
    bindChangeEvent(binder, "toggleChanged", [&](const radia::ui::ChangeEvent& event) {
    ...
    result.root->setVisibility(radia::ui::Visibility::Hidden);
    label->activate();
    ...
    const radia::ui::LayoutBuildResult required =
    ...
    const radia::ui::LayoutBuildResult invalid = factory.buildWidgetTreeFromString(kInvalidLabelTargetLayout, "label-invalid.xml");
    ...
    const radia::ui::LayoutBuildResult missing =
    ...
    const radia::ui::LayoutBuildResult wrongType =
    ...
    const radia::ui::LayoutBuildResult crossScope = factory.buildWidgetTreeFromString(kCrossScopeLabelLayout, "label-scope.xml");
    ...
    const radia::ui::LayoutBuildResult nested =
...
template<> template<> void fieldObject::test<4>() {
    ...
    radia::ui::LayoutBuildResult result = factory.buildWidgetTreeFromString(kFieldsetLayout, "fieldset.xml");
    auto* fieldset = result.rootAs<radia::ui::Fieldset>();
    ...
    auto* controlFirst = dynamic_cast<radia::ui::Field*>(fieldset->children()[2].get());
    ...
    radia::ui::layoutTree(*fieldset, stylesheet, text, radia::ui::LayoutDirection::LeftToRight);
    const radia::ui::Style legendStyle = radia::ui::resolveWidgetStyle(stylesheet, *fieldset->legend());
    ...
    const radia::ui::Widget& firstField = *fieldset->children()[1];
    const radia::ui::Widget& secondField = *fieldset->children()[2];
    ...
    const radia::ui::Style fieldsetStyle = radia::ui::resolveWidgetStyle(stylesheet, *fieldset);
    ...
    radia::ui::StyleSheet obsoletePartStylesheet;
    const radia::ui::StyleSheetLoadResult obsoletePart = obsoletePartStylesheet.loadRadia("fieldset::legend { font-size: 10px; }");
    ...
    radia::ui::StyleSheet obsoleteFieldPartStylesheet;
    const radia::ui::StyleSheetLoadResult obsoleteFieldPart = obsoleteFieldPartStylesheet.loadRadia("field::hint { font-size: 10px; }");
    ...
    radia::ui::layoutTree(*fieldset, stylesheet, text, radia::ui::LayoutDirection::RightToLeft);
    ...
    radia::ui::layoutTree(*controlFirst, stylesheet, text, radia::ui::LayoutDirection::LeftToRight);
    ensure_equals("control-first Hint aligns with Label in LTR", controlFirst->hint()->rect().left(), controlFirst->label()->rect().left());
    radia::ui::layoutTree(*controlFirst, stylesheet, text, radia::ui::LayoutDirection::RightToLeft);
    ...
    const radia::ui::LayoutBuildResult missingLegend = factory.buildWidgetTreeFromString(kMissingLegendLayout, "missing-legend.xml");
    ...
    const radia::ui::LayoutBuildResult missingField =
    ...
    const radia::ui::LayoutBuildResult duplicateLegend = factory.buildWidgetTreeFromString(kDuplicateLegendLayout, "duplicate-legend.xml");
    ...
    const radia::ui::LayoutBuildResult unsupportedChild =
    ...
    const radia::ui::LayoutBuildResult flowBreak = factory.buildWidgetTreeFromString(kFieldsetBreakLayout, "fieldset-break.xml");
    ...
    const radia::ui::LayoutBuildResult standaloneLegend = factory.buildWidgetTreeFromString("<legend>Orphan</legend>", "legend.xml");
...
template<> template<> void fieldObject::test<5>() {
    ...
    radia::ui::LayoutBuildResult result = factory.buildWidgetTreeFromString(kReorderedFieldsetLayout, "reordered-fieldset.xml");
    auto* fieldset = result.rootAs<radia::ui::Fieldset>();
    ensure("reordered Fieldset compiles", result.ok() && fieldset && fieldset->legend());
    auto* late = dynamic_cast<radia::ui::Field*>(fieldset->children()[0].get());
    auto* early = dynamic_cast<radia::ui::Field*>(fieldset->children()[2].get());
...
template<> template<> void fieldObject::test<6>() {
    radia::ui::Panel owner;
    auto existingParent = std::make_unique<radia::ui::Panel>();
    radia::ui::Panel* parent = existingParent.get();
    auto existingChild = std::make_unique<radia::ui::Panel>();
    radia::ui::Panel* child = existingChild.get();
    ...
    radia::ui::WidgetContract contract;
    radia::ui::CompositePartContract nested;
    nested.path = "parent::child";
    nested.parentPath = "parent";
    nested.create = [] { return std::make_unique<radia::ui::Panel>(); };
    radia::ui::CompositePartContract root;
    root.path = "parent";
    root.create = [] { return std::make_unique<radia::ui::Panel>(); };
