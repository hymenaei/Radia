...
namespace tut
{
    struct detached_floater_manager
    {
        struct MemoryPersistence final : rdui::viewer::FloaterPlacementStore::Persistence
        {
            ...
            void write(const LLSD& placements) override { value = placements; ++writes; }
        ...
        class FakePresentation final : public rdui::viewer::DetachedFloaterPresentation
        {
            public:
                ...
                {
                    mState.logical = {logical_rect.w, logical_rect.h};
                }
        ...
        detached_floater_manager()
            : store(persistence),
              manager(surface, store,
                  [this](std::unique_ptr<rdui::Floater>& floater)
                  {
                      return std::make_unique<FakePresentation>(std::move(floater), presentation);
                  }, environment)
        {
    ...
    template<> template<>
    void detached_floater_manager_object::test<1>()
    {
    ...
    template<> template<>
    void detached_floater_manager_object::test<2>()
    {
    ...
    template<> template<>
    void detached_floater_manager_object::test<3>()
    {
        set_test_name("invisible saved placement leaves floater attached");
        rdui::Floater& floater = mountFloater();
        environment.visiblePlacement = false;
        const bool restored = manager.restore(identity, floater,
            {100, 200, 320, 240, "missing-monitor", std::nullopt});
    ...
    template<> template<>
    void detached_floater_manager_object::test<4>()
    {
    ...
    template<> template<>
    void detached_floater_manager_object::test<5>()
    {
    ...
    template<> template<>
    void detached_floater_manager_object::test<6>()
    {
        ...
        ensure_approximately_equals("logical width persists without native rounding",
                                    persistence.value["demo"]["logical_width"].asReal(), 411.5, 6);
        ensure_approximately_equals("logical height persists without native rounding",
                                    persistence.value["demo"]["logical_height"].asReal(), 277.25, 6);
    }

    template<> template<>
    void detached_floater_manager_object::test<7>()
    {
        set_test_name("disabling resize deletes stale detached logical dimensions");
        rdui::Floater& floater = mountFloater();
        manager.requestDetach(identity, floater, {90.f, 110.f}, {});
        manager.processPendingDetach();
        ensure("resizable placement initially has logical width",
               persistence.value["demo"].has("logical_width"));
    ...
    template<> template<>
    void detached_floater_manager_object::test<9>()
    {
    ...
    template<> template<>
    void detached_floater_manager_object::test<10>()
    {
