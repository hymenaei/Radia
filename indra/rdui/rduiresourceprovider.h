...
struct ResourceLayer {
    ...
    std::string sourceNameFor(std::string_view resource_id) const {
        ...
        if (source_name.size() >= entrypoint.size()
            && source_name.compare(source_name.size() - entrypoint.size(), entrypoint.size(), entrypoint) == 0) {
            return source_name.substr(0, source_name.size() - entrypoint.size()) + std::string(resource_id);
        }
...
public:
    ...
    void add(std::string resource_id, std::string source) { mResources.insert_or_assign(std::move(resource_id), std::move(source)); }
