    ...
    class LayoutEngine
    {
        public:
            static Vec2 measure(Widget& node, const StyleSheet& theme, const TextMetrics& text_metrics,
                                std::optional<float> outer_width = std::nullopt)
            {
        ...
        private:
            ...
            static RowAllocation allocateRowLine(
                Widget& parent,
                ...
                float available_main)
            {
                ...
                if (auto_margins)
                {
                    allocation.auto_margin = std::max(0.f, allocation.free_space) / static_cast<float>(auto_margins);
                }
                return allocation;
            }

            static void remeasureRowChildren(std::vector<ChildLayout>& children, std::size_t begin, std::size_t end,
                                             const StyleSheet& theme, const TextMetrics& text_metrics)
            {
                for (std::size_t index = begin; index < end; ++index)
                {
                    ...
                    for (const auto& [begin, end] : lines)
                    {
                        ...
                        for (std::size_t index = begin; index < end; ++index)
                        {
                            applyCrossAxisSizing(children[index].measured, children[index].style, flow,
                                                 preliminary_height, crossAlignment(parent_style, children[index].style, flow));
                        }
                    }
