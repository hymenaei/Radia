...
AuxiliaryWindowRect RuntimeWindowAdapter::mainRectToNative(const rdui::Rect& rect) const {
    if (!mWindow) return {};
...
AuxiliaryWindowPoint RuntimeWindowAdapter::mainPointToNative(const rdui::Vec2& point) const {
    if (!mWindow) return {};
    const rdui::Vec2 scale = mDisplayScale ? mDisplayScale() : rdui::Vec2{1.f, 1.f};
    const rdui::Vec2 scaled = detail::scaleLogicalPoint(point, scale);
...
rdui::Vec2 RuntimeWindowAdapter::nativeBottomLeftInMain(const AuxiliaryWindowRect& rect) const {
    if (!mWindow) return {};
    const rdui::Vec2 scale = mDisplayScale ? mDisplayScale() : rdui::Vec2{1.f, 1.f};
...
bool RuntimeWindowAdapter::nativePointInsideMain(const rdui::Vec2& point) const {
...
std::optional<AuxiliaryWindowPoint> RuntimeWindowAdapter::releasePointerForDetach(const rdui::Vec2& mainPosition) {
