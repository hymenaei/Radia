/**
 * @file rduistyleproperty_test.cpp
 * @brief
 *
...
template<> template<> void rduistyleproperty_object::test<1>() {
    ...
    ensure_equals("compiled selector matches element, class, state, and child", rdui::resolveWidgetStyle(stylesheet, icon).width.pixels(), 17.f);
}

template<> template<> void rduistyleproperty_object::test<2>() {
    rdui::StyleSheet theme;
    theme.loadRadia(":root { --accent: #204060ff; --space: 12px; } button { background-color: var(--accent); padding: var(--space); "
                    "border-radius: 5px; }");
...
template<> template<> void rduistyleproperty_object::test<3>() {
    rdui::StyleSheet theme;
...
template<> template<> void rduistyleproperty_object::test<4>() {
    rdui::StyleSheet theme;
...
template<> template<> void rduistyleproperty_object::test<5>() {
    rdui::StyleSheet theme;
    ensure("nested stylesheet loads",
           theme
               .loadRadia("button { background-color: #101010ff; &:hover { background-color: #202020ff; } "
...
template<> template<> void rduistyleproperty_object::test<6>() {
    rdui::StyleSheet theme;
    ...
    ensure("cross-axis alignment compiles",
           theme
               .loadRadia("panel { align-items: end; } panel.normal { align-items: normal; } "
                          "button { align-self: start; } button.auto { align-self: auto; }")
               .ok());
    ensure_equals("container cross-axis alignment is typed", static_cast<int>(theme.resolve("panel", "", {}, 0).align_items),
                  static_cast<int>(rdui::AlignItems::End));
    ensure_equals("flow-item cross-axis override is typed", static_cast<int>(theme.resolve("button", "", {}, 0).align_self),
                  static_cast<int>(rdui::AlignSelf::Start));
    ensure_equals("normal container alignment can be authored", static_cast<int>(theme.resolve("panel", "", {"normal"}, 0).align_items),
                  static_cast<int>(rdui::AlignItems::Normal));
    ensure_equals("auto flow-item alignment can be authored", static_cast<int>(theme.resolve("button", "", {"auto"}, 0).align_self),
                  static_cast<int>(rdui::AlignSelf::Auto));
}

template<> template<> void rduistyleproperty_object::test<7>() {
    rdui::StyleSheet theme;
    ...
    ensure("numeric variable font weights compile", variable_weight.loadRadia("label { font-weight: 525; }").ok());
    ensure_equals("numeric weight is preserved", variable_weight.resolve("label", "", {}, 0).font_weight, static_cast<U16>(525));

    rdui::StyleSheet pseudo_family;
    ensure("weight is not accepted as a pseudo font family", !pseudo_family.loadRadia("label { font-family: sans-bold; }").ok());
    ensure("fractional CSS weights are rejected", !pseudo_family.loadRadia("label { font-weight: 525.5; }").ok());
    ensure("CSS weights are unitless", !pseudo_family.loadRadia("label { font-weight: 700px; }").ok());
}

template<> template<> void rduistyleproperty_object::test<8>() {
    rdui::StyleSheet theme;
    theme.loadRadia("button { border: 1px #112233ff; border-width: 2px 3px; border-color: #ffffffff; } button > icon { stroke: 4px "
                    "#abcdef88; stroke-linecap: "
                    "square; }");
    const rdui::Style style = theme.resolve("button", "", {}, 0);
...
template<> template<> void rduistyleproperty_object::test<9>() {
    rdui::StyleSheet theme;
...
template<> template<> void rduistyleproperty_object::test<10>() {
    rdui::StyleSheet theme;
...
template<> template<> void rduistyleproperty_object::test<11>() {
    rdui::StyleSheet theme;
    ensure("flow item stylesheet compiles",
           theme
               .loadRadia("panel { padding: 1px 2px 3px 4px; min-width: 20px; min-height: 10px; gap: 7px; flex: 2 3 40%; order: -2; }"
                          "panel.auto { flex: auto; } panel.none { flex: none; } panel.one { flex: 4; } panel.two { flex: 5 6; }"
                          "panel.basis { flex: 10px; }")
               .ok());
...
template<> template<> void rduistyleproperty_object::test<12>() {
    ...
    ensure_equals("default container vertical alignment is top", static_cast<int>(style.vertical_align), static_cast<int>(rdui::VerticalAlign::Top));
    rdui::StyleSheet button_theme;
    rdui::Button button;
    const rdui::Style button_defaults = rdui::resolveWidgetStyle(button_theme, button);
    ensure_equals("Button intrinsically uses row flow", static_cast<int>(button_defaults.flow), static_cast<int>(rdui::Flow::Row));
    ensure_equals("Button intrinsically centers content horizontally", static_cast<int>(button_defaults.justify_content),
                  static_cast<int>(rdui::JustifyContent::Center));
    ensure_equals("Button intrinsically aligns content to the middle", static_cast<int>(button_defaults.vertical_align),
                  static_cast<int>(rdui::VerticalAlign::Middle));
    ensure("explicit Button layout compiles", button_theme.loadRadia("button { flow: column; justify-content: end; vertical-align: top; }").ok());
    const rdui::Style authored_button = rdui::resolveWidgetStyle(button_theme, button);
    ensure_equals("authored Button flow overrides its intrinsic default", static_cast<int>(authored_button.flow),
                  static_cast<int>(rdui::Flow::Column));
    ensure_equals("authored Button justification overrides its intrinsic default", static_cast<int>(authored_button.justify_content),
                  static_cast<int>(rdui::JustifyContent::End));
    ensure_equals("authored Button vertical alignment overrides its intrinsic default", static_cast<int>(authored_button.vertical_align),
    ...
    rdui::Field field;
    ensure_equals("Field intrinsically aligns content to the middle", static_cast<int>(rdui::resolveWidgetStyle(field_theme, field).vertical_align),
                  static_cast<int>(rdui::VerticalAlign::Middle));
    ensure("explicit Field vertical alignment compiles", field_theme.loadRadia("field { vertical-align: bottom; }").ok());
    ensure_equals("authored Field vertical alignment overrides its intrinsic default",
                  static_cast<int>(rdui::resolveWidgetStyle(field_theme, field).vertical_align), static_cast<int>(rdui::VerticalAlign::Bottom));
    ensure_equals("initial box fill is presentation-neutral", style.background_color.a, 0.f);
}

template<> template<> void rduistyleproperty_object::test<13>() {
    rdui::StyleSheet theme;
    ensure("text presentation properties compile",
           theme
               .loadRadia("panel { letter-spacing: 50%; word-spacing: 25%; text-wrap: nowrap; }"
    ...
    ensure_equals("letter-spacing percentage is retained for used-value resolution", inherited.letter_spacing.percent, .5f);
    ensure_equals("word-spacing percentage is retained for used-value resolution", inherited.word_spacing.percent, .25f);
    ensure_equals("text-wrap inherits", static_cast<int>(inherited.text_wrap), static_cast<int>(rdui::TextWrap::NoWrap));
    ensure_equals("text-overflow remains local", static_cast<int>(inherited.text_overflow), static_cast<int>(rdui::TextOverflow::EllipsisCenter));
