...
float roundedRectPerimeterCoordinate(vec2 p, vec2 size, float radius)
{
    ...
    if (nearest == top_distance)
        return horizontal + r * 2.0 * half_pi + vertical + clamp(size.x - r - point.x, 0.0, horizontal);
...
vec4 blurredEffectColor(vec2 texture_coord)
{
    ...
    float amount = clamp(dot(pixel_coord - rduiEffectGradientStart, gradient_axis)
                       / max(dot(gradient_axis, gradient_axis), 1.0e-6), 0.0, 1.0);
    ...
    for (int sample_index = -max_samples_per_side;
         sample_index <= max_samples_per_side; ++sample_index)
    {
...
vec4 compositedEffectColor(vec2 texture_coord)
{
    ...
    if (rduiEffectRoundedMask != 0)
    {
        ...
        float distance = roundedRectDistance(point - rduiEffectMaskRect.xy,
                                             rduiEffectMaskRect.zw, rduiEffectMaskRadius);
...
float gradientAmount(vec2 local_coord)
{
    ...
    else if (rduiGradientKind == 1)
    {
        amount = length((local_coord - rduiGradientCenter) / max(rduiGradientRadius, vec2(1.0e-4)));
    }
    else
    {
...
vec4 filteredGradientColor(vec2 local_coord)
{
    ...
    if (rduiGradientRepeating != 0)
        repeating_total = gradientIntervalIntegral(rduiGradientStops[rduiGradientStopCount - 1]);
    ...
    if (rduiGradientKind == 2)
    {
        ...
        start_integral = start_turn * turn_total
            + underlyingGradientIntegral(start - start_turn, repeating_total) - origin;
        end_integral = end_turn * turn_total
            + underlyingGradientIntegral(end - end_turn, repeating_total) - origin;
    }
    else
    {
