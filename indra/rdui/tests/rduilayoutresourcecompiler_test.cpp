...
namespace tut
{
    ...
    template<> template<>
    void rduilayoutresourcecompiler_object::test<3>()
    {
    ...
    template<> template<>
    void rduilayoutresourcecompiler_object::test<4>()
    {
    ...
    template<> template<>
    void rduilayoutresourcecompiler_object::test<5>()
    {
        ...
        ensure_equals("mixed content order is represented once", parsed.document->root->content.size(), 3U);
               ...
               parsed.document->root->content[0].isText()
               && !parsed.document->root->content[1].isText()
               && parsed.document->root->content[2].isText());
        ensure("element source range retained",
               parsed.document->root->content[1].source.begin.line == 1
               && parsed.document->root->content[1].source.end.offset
                  >= parsed.document->root->content[1].source.begin.offset);
    }
    ...
    void rduilayoutresourcecompiler_object::test<6>()
    {
        ...
        ensure("button.label authoring syntax is removed",
               !factory.createFromString("<button><button.label value=\"old\"/></button>").ok());
        ensure("button.icon authoring syntax is removed",
               !factory.createFromString("<button><button.icon name=\"old\"/></button>").ok());
        ensure("switch.label authoring syntax is removed",
               !factory.createFromString("<switch><switch.label value=\"old\"/></switch>").ok());
        ensure("label text is no longer authored through value",
               !factory.createFromString("<label value=\"old\"/>").ok());
    }

    template<> template<>
    void rduilayoutresourcecompiler_object::test<7>()
    {
        ...
        ensure("fragment shader supports gradient borders", fragment.str().find("rduiShapeMode == 6") != std::string::npos
            && fragment.str().find("rduiBorderWidths") != std::string::npos);
    ...
    template<> template<>
    void rduilayoutresourcecompiler_object::test<8>()
    {
        rdui::System system;
        ...
        auto localized_floater = std::unique_ptr<rdui::Floater>(
            static_cast<rdui::Floater*>(result.root.release()));
    ...
    template<> template<>
    void rduilayoutresourcecompiler_object::test<9>()
    {
    ...
    void rduilayoutresourcecompiler_object::test<13>()
    {
        ...
        ensure_equals("minimize icon keeps the Floater part path",
                      floater->minimizeButton()->children()[0]->part(), "header::minimize::icon");
    ...
    void rduilayoutresourcecompiler_object::test<14>()
    {
        ...
        ensure("Widget Defaults validation reports invalid root",
               factory.validateWidgetDefaults("floater").hasErrors());
    ...
    void rduilayoutresourcecompiler_object::test<16>()
    {
        ...
        ensure_equals("Hidden value is typed", static_cast<int>(result.root->children()[1]->visibility()),
                      static_cast<int>(rdui::Visibility::Hidden));
    ...
    void rduilayoutresourcecompiler_object::test<17>()
    {
        ...
        ensure_equals("Widget Defaults preserve visibility diagnostics", visibility.errors.front().code,
                      "view.attribute.visibility_invalid");
